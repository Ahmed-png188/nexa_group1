...
function Submissions({wsId,sequences,toast_}:{wsId:string;sequences:any[];toast_:(m:string,ok?:boolean)=>void}) {
  ...
  async function addToContacts() {
    ...
    setAdding(true)
    await supabase.from('contacts').upsert(toAdd.map(s=>({...s,tags:Array.from(new Set([...(s.tags||[]),'lead']))})),{onConflict:'workspace_id,email'})
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          {liveUrl&&(
            <button onClick={()=>{navigator.clipboard.writeText(liveUrl);toast_('Link copied!')}} className="btn-secondary" style={{fontSize:12,padding:'6px 12px',display:'flex',alignItems:'center',gap:6}}>
              ...
              {isArabic ? 'انسخ الرابط' : 'Copy link'}
            </button>
          )}
          {/* AI Generate button */}
          ...
          <button onClick={save} disabled={saving}
            ...
            {saving?(isArabic?'يحفظ…':'Saving…'):saved?(isArabic?'✓ تم النشر':'✓ Published'):(isArabic?'انشر':'Publish')}
          </button>
        </div>
      </div>
            ...
            <div style={{flex:1,overflowY:'auto',padding:'18px'}}>
              ...
              {section==='design'&&(
                <>
                  ...
                  <Section title="Form language">
                    <div style={{fontSize:11,color:'var(--text-3)',marginBottom:8}}>Controls the language visitors see on the public form</div>
                    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:14}}>
                      {[{id:'auto',label:'Auto'},{id:'en',label:'English'},{id:'ar',label:'عربي'}].map(l=>(
                        <button key={l.id} onClick={()=>upd('formLang',l.id as any)}
                          style={{padding:'9px',borderRadius:'var(--r)',background:cfg.formLang===l.id?'var(--cyan-dim)':'rgba(255,255,255,0.03)',border:`1px solid ${cfg.formLang===l.id?'var(--cyan-border)':'var(--border)'}`,cursor:'pointer',fontSize:12,fontWeight:cfg.formLang===l.id?600:400,color:cfg.formLang===l.id?'var(--cyan)':'var(--text-2)',transition:'all 0.15s'}}>
                          {l.label}
                        </button>
                      ))}
                    </div>
