    ...
    grow: {
      ...
      features:[
        ...
        },
        {
          ...
          desc:'Import, segment, and auto-enroll contacts',
        },
        {
...
function Label({ children }: { children: any }) {
  ...
  const font = isArabic ? "'Tajawal', system-ui, sans-serif" : F_DEFAULT
  return <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase' as const, color:'rgba(0,170,255,0.70)', marginBottom:'8px', fontFamily:font }}>{children}</div>
}
      ...
      <aside style={{ width:220, borderRight:'1px solid rgba(255,255,255,0.08)', display:'flex', flexDirection:'column', flexShrink:0, background:'#141414' }}>
              ...
              <div>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:2 }}>
                  ...
                  <button onClick={()=>{ setSenderNameDraft(senderName); setEditingSenderName(true) }}
                    style={{ background:'transparent', border:'none', cursor:'pointer', padding:0, fontSize:'10px', color:'rgba(255,255,255,0.28)', fontFamily:F }}>{isArabic ? 'عدّل' : 'Edit'}</button>
                </div>
        ...
        <div style={{ padding:'8px 12px 12px' }}>
          <button onClick={() => fileRef.current?.click()} disabled={csvImporting}
            ...
            {csvImporting ? (isArabic?'يستورد…':'Importing…') : (isArabic?'استورد CSV':'Import CSV')}
          </button>
            ...
            <div style={{ width:260, borderRight:'1px solid rgba(255,255,255,0.08)', display:'flex', flexDirection:'column', flexShrink:0, background:'#141414' }}>
              <div style={{ padding:'16px 14px', borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
                ...
                <button onClick={() => { setShowAiModal(true); setAiGoal(''); setSeqGenError(null) }}
                  ...
                  <span style={{ display:'flex' }}>{Ic.bolt}</span>{isArabic?'تسلسل جديد بالذكاء':'New sequence with AI'}
                </button>
              </div>
              <div className="ns" style={{ flex:1, overflowY:'auto' }}>
                {sequences.length === 0 ? (
                  <div style={{ padding:'32px 16px', textAlign:'center' as const, fontSize:'12px', color:'rgba(255,255,255,0.22)', fontFamily:F, lineHeight:1.7 }}>{isArabic ? 'ما في تسلسلات بعد.' : 'No sequences yet. Create one with AI above.'}</div>
                ) : sequences.map(seq => (
              ...
              {!selectedSeq ? (
                <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100%', flexDirection:'column', gap:'14px' }}>
                  ...
                  <button onClick={() => { setShowAiModal(true); setAiGoal(''); setSeqGenError(null) }}
                    ...
                    <span style={{ display:'flex' }}>{Ic.bolt}</span> {isArabic ? 'أنشئ أول تسلسل' : 'Create your first sequence'}
                  </button>
                </div>
              ) : (
        ...
        {view === 'contacts' && (
          <div className="ns a-in" style={{ flex:1, overflowY:'auto', padding:'28px 36px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
              <div>
                <h2 style={{ fontSize:'20px', fontWeight:600, color:'#FFFFFF', letterSpacing:'-0.03em', marginBottom:'4px', fontFamily:F }}>{isArabic ? 'جهات الاتصال' : 'Contacts'}</h2>
            ...
            {contacts.length === 0 ? (
              <div style={{ textAlign:'center' as const, padding:'64px 20px' }}>
                ...
                <div style={{ fontSize:'13px', color:'rgba(255,255,255,0.22)', marginBottom:'24px', fontFamily:F, lineHeight:1.65 }}>{isArabic?'استورد CSV أو ضبط نموذج العملاء لجمع جهات الاتصال تلقائياً':'Import a CSV or set up your lead form to collect contacts automatically'}</div>
                <button onClick={() => fileRef.current?.click()} style={{ padding:'10px 24px', background:'#FFFFFF', border:'none', borderRadius:'8px', fontFamily:F, fontSize:'13px', fontWeight:600, color:'#0C0C0C', cursor:'pointer' }}>{isArabic ? 'استورد CSV' : 'Import CSV'}</button>
              </div>
            ) : (
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', overflow:'hidden' }}>
          ...
          <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.12)', borderTop:'2px solid #00AAFF', borderRadius:'12px', padding:'24px', width:'100%', maxWidth:'440px', maxHeight:'80vh', overflow:'auto' }}>
            ...
            <div style={{ display:'flex', gap:'8px', justifyContent:'flex-end' }}>
              ...
              <button onClick={async()=>{
                ...
                try{const res=await fetch('/api/email/sequences/enroll',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sequence_id:selectedSeq.id,contact_ids:enrollSelected,workspace_id:workspaceId})});if(res.ok){setShowEnrollModal(false);setEnrollSelected([])}}catch{}finally{setEnrolling(false)}
        ...
        <div style={{ position:'fixed', inset:0, zIndex:300, background:'rgba(0,0,0,0.80)', backdropFilter:'blur(14px)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
          ...
          <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.12)', borderTop:'2px solid #00AAFF', borderRadius:'16px', padding:'28px', width:'100%', maxWidth:'500px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'24px' }}>
              ...
              <div>
                <div style={{ fontSize:'17px', fontWeight:700, color:'#FFFFFF', letterSpacing:'-0.03em', fontFamily:F }}>{isArabic ? 'أنشئ تسلسل بالذكاء' : 'Create sequence with AI'}</div>
