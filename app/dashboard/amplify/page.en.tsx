...
function AmplifyInner() {
  ...
  async function init() {
    ...
    const [{ data: wsData }, { data: conn }, { data: camps }, { data: content }] = await Promise.all([
    ...
    setWs(wsData)
    const plan = (wsData as any)?.plan ?? 'spark'
    const status = (wsData as any)?.plan_status ?? 'trialing'
    const isTrial = status === 'trialing'
    const trialExpired = isTrial && (new Date() > new Date(new Date((wsData as any)?.created_at).getTime() + 7 * 86400000))
    const amplifyPlans = ['grow','scale','agency']
    setHasAmplify(!trialExpired && amplifyPlans.includes(plan))

    setConnected(!!conn)
    setBrandName((wsData as any)?.brand_name || (wsData as any)?.name || 'Your Brand')
  ...
  /* ─── PLAN GATE — Amplify requires Grow+ ─── */
    ...
    <div style={{ height:'calc(100vh - var(--topbar-h))', overflowY:'auto', backgroundImage:'url(/cyan-header.png)', backgroundSize:'cover', backgroundPosition:'center', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 32px', position:'relative' }}>
      <div style={{ position:'relative', zIndex:1, textAlign:'center', maxWidth:620, animation:'pageUp 0.4s cubic-bezier(0.22,1,0.36,1) both' }}>
        <div style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'6px 14px', borderRadius:999, background:'rgba(0,0,0,0.12)', border:'1px solid rgba(0,0,0,0.16)', marginBottom:32, backdropFilter:'blur(8px)' }}>
          ...
          <span style={{ fontSize:12, fontWeight:600, color:'rgba(0,0,0,0.65)', letterSpacing:'0.02em' }}>{isArabic ? 'Amplify · غير مفعّل' : 'Amplify · Not enabled'}</span>
        </div>
        <h1 style={{ fontSize:'clamp(32px,5vw,52px)', fontWeight:800, letterSpacing:'-0.04em', color:'#0C0C0C', lineHeight:1.1, marginBottom:16, fontFamily:F }}>
          {isArabic ? <>حوّل محتواك<br/>إلى إعلانات تبيع.</> : <>Turn your content<br/>into ads that sell.</>}
        </h1>
        <p style={{ fontSize:16, color:'rgba(0,0,0,0.52)', lineHeight:1.75, maxWidth:480, margin:'0 auto 40px', fontFamily:F }}>
          {isArabic
            ? 'حوّل أفضل منشوراتك لإعلانات Meta بنقرة واحدة. كوبي بالذكاء الاصطناعي، كريتيف يعكس براندك، ونشر مباشر. متاح من خطة Grow.'
            : 'Turn your best posts into Meta Ads in one click. AI-written copy, creatives that match your brand, direct publishing. Available on Grow and above.'}
        </p>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:40 }}>
          {(isArabic ? [
            ...
            { icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>, title:'One-click boost', desc:'Turn any Studio post into a Meta ad in seconds' },
            { icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg>, title:'AI-written copy', desc:'Nexa writes ad copy in your brand voice and optimises it' },
            { icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>, title:'Performance monitor', desc:'Daily briefs on spend, reach, and conversions' },
          ]).map(f => (
            <div key={f.title} style={{ padding:'20px 18px', borderRadius:14, background:'rgba(255,255,255,0.72)', border:'1px solid rgba(255,255,255,0.90)', backdropFilter:'blur(16px)', textAlign:isArabic?'right':'left' }}>
              <div style={{ color:'rgba(0,0,0,0.55)', marginBottom:12, display:'flex', justifyContent:isArabic?'flex-end':'flex-start' }}>{f.icon}</div>
              <div style={{ fontSize:14, fontWeight:700, color:'#0C0C0C', marginBottom:6, letterSpacing:'-0.02em', fontFamily:F }}>{f.title}</div>
              <div style={{ fontSize:12, color:'rgba(0,0,0,0.50)', lineHeight:1.65, fontFamily:F }}>{f.desc}</div>
            </div>
          ))}
        </div>
        <a href="/dashboard/settings?tab=billing"
          style={{ display:'inline-flex', alignItems:'center', gap:9, padding:'14px 32px', fontSize:15, fontWeight:700, fontFamily:F, letterSpacing:isArabic?0:'-0.02em', background:'#0C0C0C', color:'#ffffff', borderRadius:12, textDecoration:'none', transition:'all 0.18s', boxShadow:'0 4px 24px rgba(0,0,0,0.30)' }}
        ...
        <div style={{ marginTop:14, fontSize:12, color:'rgba(0,0,0,0.45)', fontFamily:F }}>
          {isArabic ? '$89/شهر · إلغاء في أي وقت · بدون رسوم إعداد' : '$89/mo · Cancel anytime · No setup fees'}
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ height:'calc(100vh - var(--topbar-h))', overflowY:'auto', background:'#0C0C0C', fontFamily:F }}>
      ...
      <div style={{ padding:'28px 36px 20px', display:'flex', alignItems:'flex-start', justifyContent:'space-between' }}>
        <div>
          <h1 style={{ fontFamily:F, fontSize:'22px', fontWeight:700, letterSpacing:'-0.02em', color:'#FFFFFF', marginBottom:'4px' }}>{isArabic ? 'Amplify' : 'Amplify'}</h1>
          <p style={{ fontSize:'13px', color:'rgba(255,255,255,0.42)', fontFamily:F }}>{isArabic?'.إعلانات Meta مدعومة بـ Brand Brain. بدون وكالة':'Meta Ads powered by Brand Brain. No agency needed.'}</p>
        </div>
      ...
      {!connected && (
        <div style={{ background:'rgba(0,170,255,0.05)', border:'1px solid rgba(0,170,255,0.20)', borderRadius:'10px', padding:'14px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', margin:'0 36px 20px', gap:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              <div style={{ fontFamily:F, fontSize:13, fontWeight:700, color:'#FFFFFF', marginBottom:2 }}>{isArabic?'وصّل حساب Meta':'Connect your Meta account'}</div>
          ...
          <button onClick={() => {
            ...
            {isArabic ? 'وصّل Meta ←' : 'Connect Meta →'}
          </button>
        </div>
      )}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px', marginBottom:'24px', padding:'0 36px' }}>
        {[
          { label: isArabic?'الإنفاق الكلي':'Total spent',      value: totalSpend>0?`$${totalSpend.toFixed(0)}`:'—',   sub:'This month',    color:'#00AAFF' },
          { label: isArabic?'الوصول':'People reached',   value: totalReach>0?(totalReach>=1000?`${(totalReach/1000).toFixed(1)}k`:String(totalReach)):'—', sub:'Unique people', color:'#22C55E' },
          { label: isArabic?'النقرات':'Link clicks',      value: totalClicks>0?String(totalClicks):'—', sub: totalClicks>0&&totalSpend>0?`Avg $${(totalSpend/totalClicks).toFixed(2)} per click`:'No clicks yet', color:'#00AAFF' },
          { label: isArabic?'الحملات النشطة':'Active campaigns', value: String(activeCamps), sub:`${campaigns.length} total`, color:'#FB923C' },
        ].map((s,i) => (
          ...
          <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'12px', padding:'24px' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
              <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', fontFamily:F }}>
                {adMode==='simple' ? isArabic?'إطلاق سريع — الذكاء يتولى كل شيء':'Quick launch — AI handles targeting & copy' : isArabic?'الوضع المتقدم — تحكم كامل':'Advanced mode — full control'}
              </div>
            ...
            {adMode === 'simple' && (
              <div>
                ...
                <div style={{ marginBottom:20 }}>
                  <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', fontFamily:F, marginBottom:10 }}>{isArabic?'١. اختر أو ارفع محتوى':'1. Pick or upload content'}</div>
                ...
                <div style={{ marginBottom:20 }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
                    <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', fontFamily:F }}>{isArabic?'٢. نص الإعلان':'2. Ad copy'}</div>
                    <button onClick={generateAdCopy} disabled={generatingCopy||(!selectedContent&&!adCopy.trim()&&!uploadedImage)}
                      ...
                      {generatingCopy ? <><div className="nexa-spinner" style={{ width:9, height:9 }}/>{isArabic?'يكتب...':'Writing…'}</> : <>{isArabic?'اكتب بالذكاء':'Write with AI'}</>}
                    </button>
                  </div>
                ...
                {/* 3 — Budget */}
                <div style={{ marginBottom:20 }}>
                  <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', fontFamily:F, marginBottom:10 }}>{isArabic?'٣. الميزانية اليومية':'3. Daily budget'}</div>
                ...
                <button onClick={handleLaunch} disabled={launching||!workspaceId}
                  ...
                  {launching ? <><div className="nexa-spinner" style={{ width:13, height:13, borderColor:'rgba(0,0,0,0.15)', borderTopColor:'#0C0C0C' }}/>{isArabic?'يطلق...':'Launching…'}</> : <>Launch campaign &mdash; ${dailyBudget}/day &rarr;</>}
                </button>
                <div style={{ marginTop:8, fontSize:'11px', color:'rgba(255,255,255,0.25)', fontFamily:F, textAlign:'center' }}>{isArabic?'الذكاء يحدد الجمهور والمواضع':'AI sets audience & placements'} based on your Brand Brain</div>
              </div>
            )}
            ...
            {adMode === 'advanced' && (
              <div>
                ...
                {currentStep === 1 && (
                  <div>
                    <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', fontFamily:F, marginBottom:14 }}>{isArabic?'الكريتيف — النص والصورة':'Creative — Copy & Image'}</div>
                    ...
                    <button onClick={generateAdCopy} disabled={generatingCopy}
                      ...
                      {generatingCopy?<><div className="nexa-spinner" style={{ width:10, height:10 }}/>Writing&hellip;</>:<>{isArabic?'اكتب بـ Brand Brain':'Write copy with Brand Brain'}</>}
                    </button>
                  </div>
                )}
                ...
                {currentStep === 2 && (
                  <div>
                    <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', fontFamily:F, marginBottom:14 }}>{isArabic?'هدف الحملة':'Campaign objective'}</div>
                ...
                {currentStep === 3 && (
                  <div>
                    <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', fontFamily:F, marginBottom:14 }}>{isArabic?'استهداف الجمهور':'Audience targeting'}</div>
                    <div style={{ marginBottom:14 }}>
                ...
                {currentStep === 5 && (
                  <div>
                    <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', fontFamily:F, marginBottom:14 }}>{isArabic?'الميزانية والجدول':'Budget & schedule'}</div>
                ...
                {currentStep === 6 && (
                  <div>
                    <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', fontFamily:F, marginBottom:14 }}>{isArabic?'راجع قبل الإطلاق':'Review before launching'}</div>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', marginTop:20 }}>
                  {currentStep > 1 ? <button onClick={()=>setCurrentStep(s=>s-1)} style={{ padding:'9px 18px', background:'transparent', border:'1px solid rgba(255,255,255,0.10)', borderRadius:8, fontSize:'12px', color:'rgba(255,255,255,0.40)', cursor:'pointer', fontFamily:F }}>{isArabic?'← رجوع':'← Back'}</button> : <span/>}
                  {currentStep < 6 && <button onClick={()=>setCurrentStep(s=>s+1)} style={{ padding:'9px 20px', background:'#FFFFFF', border:'none', borderRadius:8, fontSize:'12px', fontWeight:600, color:'#0C0C0C', cursor:'pointer', fontFamily:F }}>{isArabic?'تابع →':'Continue →'}</button>}
                </div>
              </div>
            )}
          </div>
      ...
      {activeTab === 'monitor' && (
        <div style={{ padding:'0 36px 36px' }}>
          <div style={{ marginBottom:20 }}>
            <div style={{ fontSize:'14px', fontWeight:600, color:'rgba(255,255,255,0.70)', fontFamily:F, marginBottom:4 }}>{isArabic?'مونيتور الحملات':'Daily Campaign Monitor'}</div>
          ...
          {!monitorLoading && monitorData.length > 0 && (
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              ...
              <button onClick={loadMonitor} style={{ padding:'8px', background:'transparent', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, fontSize:'12px', color:'rgba(255,255,255,0.35)', cursor:'pointer', fontFamily:F }}>{isArabic?'حدّث التحليل':'Refresh analysis'}</button>
            </div>
          )}
        </div>
      )}
        ...
        <div style={{ padding:'0 36px 36px' }}>
          ...
          {campaigns.length === 0 ? (
            <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'48px 24px', textAlign:'center' }}>
              ...
              <div style={{ fontFamily:F, fontSize:'18px', fontWeight:700, color:'#FFFFFF', letterSpacing:'-0.02em', marginBottom:8 }}>{isArabic ? 'ما في حملات بعد' : 'No campaigns yet'}</div>
                          ...
                          {camp.status !== 'DELETED' && (
                            editingCamp === camp.id ? (
                              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                                ...
                                <button onClick={()=>updateCampaignBudget(camp.id, editBudget)} style={{ padding:'6px 12px', background:'#FFFFFF', border:'none', borderRadius:6, fontSize:'12px', fontWeight:600, color:'#0C0C0C', cursor:'pointer', fontFamily:F }}>{isArabic?'احفظ':'Save'}</button>
                                <button onClick={()=>setEditingCamp(null)} style={{ padding:'6px 10px', background:'transparent', border:'1px solid rgba(255,255,255,0.10)', borderRadius:6, fontSize:'12px', color:'rgba(255,255,255,0.35)', cursor:'pointer', fontFamily:F }}>{isArabic?'إلغاء':'Cancel'}</button>
                              </div>
                            ) : (
...
export default function AmplifyPage() {
  return (
    <Suspense fallback={
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'calc(100vh - var(--topbar-h))', flexDirection:'column', gap:14, background:'#0C0C0C' }}>
        <div className="nexa-spinner" style={{ width:20, height:20 }}/>
        <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.25)', fontFamily:F_DEFAULT }}>Loading</div>
      </div>
    }>
      <AmplifyInner/>
    </Suspense>
