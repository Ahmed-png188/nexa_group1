...
export default function AgencyPage() {
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'calc(100vh - var(--topbar-h))', flexDirection:'column', gap:16, background:'#0C0C0C' }}>
  ...
  if (!isAgency) return (
    <div style={{
      ...
      <div style={{ position:'relative', zIndex:1, textAlign:'center', maxWidth:620, animation:'pageUp 0.4s cubic-bezier(0.22,1,0.36,1) both' }}>
        ...
        {/* Main headline */}
        <h1 style={{ fontSize:'clamp(32px,5vw,52px)', fontWeight:800, letterSpacing:'-0.04em', color:'#0C0C0C', lineHeight:1.1, marginBottom:16, fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif" }}>
          ...
          {[
            {
              ...
              title: isArabic?'مساحات العملاء':'Client workspaces',
            ...
            },
            {
              ...
              title: isArabic?'تتبع الاشتراكات':'Retainer tracking',
            ...
            },
            {
              ...
              title: isArabic?'انتقال بنقرة':'One-click switching',
        ...
        <Link
          ...
          style={{ display:'inline-flex', alignItems:'center', gap:9, padding:'14px 32px', fontSize:15, fontWeight:700, fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", letterSpacing:'-0.02em', background:'#0C0C0C', color:'#ffffff', borderRadius:12, textDecoration:'none', transition:'all 0.18s', boxShadow:'0 4px 24px rgba(0,0,0,0.30)' }}
  ...
  /* ─── CLIENT DETAIL ─── */
  if (sel) return (
    <div style={{ padding:'28px 32px 48px', height:'calc(100vh - var(--topbar-h))', overflowY:'auto', background:'#0C0C0C' }}>
      {/* Back + header */}
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:28 }}>
        ...
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <h1 style={{ fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", fontSize:22, fontWeight:800, letterSpacing:'-0.04em', color:'rgba(255,255,255,0.92)', lineHeight:1 }}>
              {sel.client_name}
            </h1>
            <span style={{ fontSize:9, fontWeight:700, padding:'2px 9px', borderRadius:100, background:sel.status==='active'?'rgba(34,197,94,0.10)':'rgba(255,255,255,0.05)', border:`1px solid ${sel.status==='active'?'rgba(34,197,94,0.20)':'rgba(255,255,255,0.1)'}`, color:sel.status==='active'?'#34D399':'rgba(255,255,255,0.35)', textTransform:'uppercase', letterSpacing:'0.05em' }}>
              {sel.status||'active'}
            </span>
          </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(160px,1fr))', gap:8, marginBottom:24 }}>
        {[
          { label: isArabic?'الاشتراك الشهري':'Monthly retainer', value:sel.monthly_retainer?`$${sel.monthly_retainer?.toLocaleString()}/mo`:'—', color:'#22C55E' },
          { label: isArabic?'محتوى مُنشأ':'Content created',  value:String(sel.content_count||0),   color:'#A78BFA' },
          { label: isArabic?'نشط منذ':'Active since',     value:sel.created_at?.slice(0,10)||'—', color:'#00AAFF' },
          { label: isArabic?'الحالة':'Status',           value:sel.status||'active',           color:sel.status==='active'?'#34D399':'#FFB547' },
        ].map(s => (
      ...
      {sel.client_workspace_id && (
        <div style={{ padding:'20px 24px', background:'linear-gradient(145deg, rgba(77,159,255,0.08) 0%, rgba(77,159,255,0.03) 100%)', border:'1px solid rgba(77,159,255,0.2)', borderRadius:16, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <button onClick={() => switchWs(sel.client_workspace_id)}
            style={{ display:'flex', alignItems:'center', gap:8, padding:'11px 22px', fontSize:13, fontWeight:700, fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", letterSpacing:'-0.02em', background:'#00AAFF', color:'#0C0C0C', border:'none', borderRadius:11, cursor:'pointer', boxShadow:'0 4px 18px rgba(77,159,255,0.35)', transition:'all 0.18s', flexShrink:0 }}>
            <span style={{ display:'flex' }}>{Ic.bolt}</span>{isArabic?'انتقل للمساحة':'Switch workspace'}
          </button>
        </div>
      )}
    </div>
  ...
  return (
    <>
      <div style={{
        padding:'28px 32px 48px', height:'calc(100vh - var(--topbar-h))', overflowY:'auto', background:'#0C0C0C',
        opacity:mounted?1:0, transition:'opacity 0.4s ease',
      }}>
        {/* Header */}
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28 }}>
          <div>
            <h1 style={{ fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", fontSize:22, fontWeight:800, letterSpacing:'-0.03em', color:'rgba(255,255,255,0.92)', lineHeight:1, marginBottom:5 }}>{isArabic ? 'الوكالة' : 'Agency'}</h1>
            <p style={{ fontSize:12, color:'rgba(255,255,255,0.3)' }}>
              {activeCount} active client{activeCount!==1?'s':''} · ${totalMRR.toLocaleString()} MRR
            </p>
          </div>
          <button onClick={() => setShowNew(true)}
            style={{ display:'flex', alignItems:'center', gap:8, padding:'9px 18px', fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", fontWeight:700, fontSize:13, letterSpacing:'-0.02em', background:'#00AAFF', color:'#0C0C0C', border:'none', borderRadius:11, cursor:'pointer', boxShadow:'0 4px 18px rgba(77,159,255,0.35)', transition:'all 0.18s' }}
            ...
            <span style={{ display:'flex' }}>{Ic.plus}</span>{isArabic ? 'أضف عميل' : 'Add client'}
          </button>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(170px,1fr))', gap:8, marginBottom:24 }}>
          {[
            { label: isArabic?'عملاء نشطون':'Active clients',  value:activeCount,                        color:'#22C55E', icon:Ic.users  },
            { label: isArabic?'دعوات معلقة':'Pending invites', value:invites.length,                     color:'#FFB547', icon:Ic.arrow  },
            { label: isArabic?'الإيراد الشهري':'Monthly revenue', value:`$${totalMRR.toLocaleString()}`,    color:'#00AAFF', icon:Ic.dollar },
            { label: isArabic?'إجمالي العملاء':'Total clients',   value:clients.length,                     color:'#A78BFA', icon:Ic.chart  },
          ].map(s => (
            <div key={s.label} className="nexa-card" style={{ padding:'14px 18px', borderRadius:14, display:'flex', alignItems:'center', gap:11 }}>
              ...
              <div>
                <div className="nexa-num" style={{ fontFamily:"'Geist Mono', monospace", fontWeight:300, fontSize:28, letterSpacing:'-0.04em', color:s.color }}>{s.value}</div>
                <div className="nexa-label" style={{ fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", fontSize:9, fontWeight:600, letterSpacing:'0.1em', textTransform:'uppercase' as const, color:'rgba(255,255,255,0.38)', marginTop:3 }}>{s.label}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {clients.length > 0 ? (
                ...
                <div key={c.id}
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                      ...
                      <div>
                        <div style={{ fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", fontWeight:600, fontSize:14, color:'rgba(255,255,255,0.9)', letterSpacing:'-0.02em', marginBottom:2 }}>{c.client_name}</div>
                        <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)' }}>{c.brand_name||'No brand set'}</div>
                      </div>
                    </div>
                    <span style={{ fontSize:9, fontWeight:700, padding:'3px 9px', borderRadius:100, background:c.status==='active'?'rgba(34,197,94,0.10)':'rgba(255,255,255,0.05)', border:`1px solid ${c.status==='active'?'rgba(34,197,94,0.20)':'rgba(255,255,255,0.09)'}`, color:c.status==='active'?'#34D399':'rgba(255,255,255,0.35)', textTransform:'uppercase', letterSpacing:'0.05em' }}>
                      {c.status||'active'}
                    </span>
                  </div>
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    <div>
                      {c.monthly_retainer > 0 ? (
                        <div style={{ fontFamily:"'Geist Mono', monospace", fontWeight:300, fontSize:13, color:'rgba(255,255,255,0.72)', letterSpacing:0, lineHeight:1 }}>
                        ...
                        <div style={{ fontSize:12, color:'rgba(255,255,255,0.28)' }}>{isArabic?'ما في اشتراك ثابت':'No retainer set'}</div>
                      )}
                    </div>
                    {c.client_workspace_id && (
                      <button
                        onClick={e => { e.stopPropagation(); switchWs(c.client_workspace_id) }}
                        style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 13px', borderRadius:9, fontSize:11, fontWeight:700, background:'rgba(0,170,255,0.10)', border:'1px solid rgba(77,159,255,0.22)', color:'#00AAFF', cursor:'pointer', fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", transition:'all 0.15s', letterSpacing:'-0.01em' }}
                        ...
                        onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background='rgba(0,170,255,0.10)';(e.currentTarget as HTMLElement).style.borderColor='rgba(0,170,255,0.20)'}}>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'44vh', textAlign:'center', padding:'40px', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:18 }}>
            <div style={{ width:56, height:56, borderRadius:16, background:'rgba(0,170,255,0.10)', border:'1px solid rgba(77,159,255,0.15)', display:'flex', alignItems:'center', justifyContent:'center', color:'#00AAFF', marginBottom:20 }}>
              {Ic.users}
            </div>
            <h3 style={{ fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", fontWeight:700, fontSize:20, letterSpacing:'-0.02em', color:'rgba(255,255,255,0.85)', marginBottom:9 }}>{isArabic ? 'ما في عملاء بعد' : 'No clients yet'}</h3>
            <p style={{ fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", fontSize:14, lineHeight:1.75, color:'rgba(255,255,255,0.32)', maxWidth:360, marginBottom:22 }}>
              Add your first client to start managing their brand, content, and workspace from one place.
            </p>
            <button onClick={() => setShowNew(true)}
              style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 26px', fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", fontWeight:700, fontSize:13, letterSpacing:'-0.02em', background:'#00AAFF', color:'#0C0C0C', border:'none', borderRadius:11, cursor:'pointer', boxShadow:'0 4px 22px rgba(77,159,255,0.38)', transition:'all 0.18s' }}>
              <span style={{ display:'flex' }}>{Ic.plus}</span>{isArabic?'أضف أول عميل':'Add first client'}
            </button>
          </div>
        )}
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.72)', backdropFilter:'blur(14px)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
          ...
          <div style={{ background:'rgba(10,10,18,0.98)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:20, padding:'26px', width:'100%', maxWidth:440, boxShadow:'0 32px 80px rgba(0,0,0,0.85)', animation:'pageUp 0.22s ease both' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
              <div>
                <div style={{ fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", fontSize:17, fontWeight:800, letterSpacing:'-0.03em', color:'rgba(255,255,255,0.92)', lineHeight:1, marginBottom:3 }}>
                  Add new client
                </div>
            ...
            {/* Form */}
            {[
              { label: isArabic?'اسم العميل':'Client name',           val:cName,     set:setCName,     ph:'Ahmed Al-Rashidi',  type:'text',   req:true  },
              { label: isArabic?'الإيميل':'Email',                 val:cEmail,    set:setCEmail,    ph:'ahmed@brand.com',   type:'email',  req:false },
              { label: isArabic?'اسم البراند / الشركة':'Brand / company name',  val:cBrand,    set:setCBrand,    ph:'Brand Inc.',        type:'text',   req:false },
              { label: isArabic?'الاشتراك الشهري ($)':'Monthly retainer ($)',  val:cRetainer, set:setCRetainer, ph:'2,500',             type:'number', req:false },
            ].map(f => (
              <div key={f.label} style={{ marginBottom:14 }}>
                ...
                <input
                  ...
                  placeholder={f.ph}
                  style={{ width:'100%', padding:'11px 13px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10, color:'rgba(255,255,255,0.88)', fontSize:13, fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", outline:'none', transition:'border-color 0.15s', boxSizing:'border-box' as const }}
            ...
            <button onClick={addClient} disabled={!cName.trim()||creating}
              style={{ width:'100%', padding:'14px', fontSize:14, fontWeight:700, fontFamily:isArabic ? "'Tajawal', system-ui, sans-serif" : "'Geist', -apple-system, sans-serif", letterSpacing:'-0.02em', background:cName.trim()?'#4D9FFF':'rgba(255,255,255,0.04)', color:cName.trim()?'#000':'rgba(255,255,255,0.2)', border:'none', borderRadius:12, cursor:cName.trim()?'pointer':'not-allowed', display:'flex', alignItems:'center', justifyContent:'center', gap:9, marginTop:8, transition:'all 0.18s', boxShadow:cName.trim()?'0 4px 22px rgba(77,159,255,0.38)':'none' }}>
              {creating
                ? <><div className="nexa-spinner" style={{ width:14, height:14 }}/>{isArabic?'يضيف العميل...':'Adding client…'}</>
                : <><span style={{ display:'flex' }}>{Ic.plus}</span>{isArabic?'أضف عميل':'Add client'}</>}
            </button>
          </div>
        </div>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position:'fixed', bottom:24, right:24, zIndex:9999, padding:'12px 20px', borderRadius:12, background:toast.ok?'rgba(34,197,94,0.10)':'rgba(239,68,68,0.10)', border:`1px solid ${toast.ok?'rgba(34,197,94,0.20)':'rgba(239,68,68,0.20)'}`, color:toast.ok?'#34D399':'#FF5757', fontSize:13, fontWeight:600, backdropFilter:'blur(16px)', boxShadow:'0 8px 32px rgba(0,0,0,0.45)', animation:'pageUp 0.2s ease both' }}>
          {toast.msg}
        </div>
      )}
    </>
