...
export default function AgencyPage() {
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'calc(100vh - var(--topbar-h))', flexDirection:'column', gap:16, background:'#0C0C0C' }}>
      ...
      <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)', fontFamily:"'Geist', -apple-system, sans-serif", letterSpacing:'0.06em', textTransform:'uppercase' as const, fontWeight:500 }}>Loading</div>
    </div>
  )
  ...
  if (!isAgency) return (
    <div style={{
      ...
      <div style={{ position:'relative', zIndex:1, textAlign:'center', maxWidth:620, animation:'pageUp 0.4s cubic-bezier(0.22,1,0.36,1) both' }}>
        ...
        {/* Main headline */}
        <h1 style={{ fontSize:'clamp(32px,5vw,52px)', fontWeight:800, letterSpacing:'-0.04em', color:'#0C0C0C', lineHeight:1.1, marginBottom:16, fontFamily:"'Geist', -apple-system, sans-serif" }}>
        ...
        <Link
          ...
          style={{ display:'inline-flex', alignItems:'center', gap:9, padding:'14px 32px', fontSize:15, fontWeight:700, fontFamily:"'Geist', -apple-system, sans-serif", letterSpacing:'-0.02em', background:'#0C0C0C', color:'#ffffff', borderRadius:12, textDecoration:'none', transition:'all 0.18s', boxShadow:'0 4px 24px rgba(0,0,0,0.30)' }}
  ...
  if (sel) return (
    <div style={{ padding:'28px 32px 48px', height:'calc(100vh - var(--topbar-h))', overflowY:'auto', background:'#0C0C0C' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:28 }}>
        ...
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <h1 style={{ fontFamily:"'Geist', -apple-system, sans-serif", fontSize:22, fontWeight:800, letterSpacing:'-0.04em', color:'rgba(255,255,255,0.92)', lineHeight:1 }}>
              {sel.client_name}
            </h1>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(160px,1fr))', gap:8, marginBottom:24 }}>
        {[
          { label:'Monthly retainer', value:sel.monthly_retainer?`$${sel.monthly_retainer?.toLocaleString()}/mo`:'—', color:'#22C55E' },
          { label:'Content created',  value:String(sel.content_count||0),   color:'#A78BFA' },
          { label:'Active since',     value:sel.created_at?.slice(0,10)||'—', color:'#00AAFF' },
      ...
      {sel.client_workspace_id && (
        <div style={{ padding:'20px 24px', background:'linear-gradient(145deg, rgba(77,159,255,0.08) 0%, rgba(77,159,255,0.03) 100%)', border:'1px solid rgba(77,159,255,0.2)', borderRadius:16, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <button onClick={() => switchWs(sel.client_workspace_id)}
            style={{ display:'flex', alignItems:'center', gap:8, padding:'11px 22px', fontSize:13, fontWeight:700, fontFamily:"'Geist', -apple-system, sans-serif", letterSpacing:'-0.02em', background:'#00AAFF', color:'#0C0C0C', border:'none', borderRadius:11, cursor:'pointer', boxShadow:'0 4px 18px rgba(77,159,255,0.35)', transition:'all 0.18s', flexShrink:0 }}>
    ...
    <>
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28 }}>
          <div>
            <h1 style={{ fontFamily:"'Geist', -apple-system, sans-serif", fontSize:22, fontWeight:800, letterSpacing:'-0.03em', color:'rgba(255,255,255,0.92)', lineHeight:1, marginBottom:5 }}>Agency</h1>
            <p style={{ fontSize:12, color:'rgba(255,255,255,0.3)' }}>
              {activeCount} active client{activeCount!==1?'s':''} · ${totalMRR.toLocaleString()} MRR
            </p>
          </div>
          <button onClick={() => setShowNew(true)}
            style={{ display:'flex', alignItems:'center', gap:8, padding:'9px 18px', fontFamily:"'Geist', -apple-system, sans-serif", fontWeight:700, fontSize:13, letterSpacing:'-0.02em', background:'#00AAFF', color:'#0C0C0C', border:'none', borderRadius:11, cursor:'pointer', boxShadow:'0 4px 18px rgba(77,159,255,0.35)', transition:'all 0.18s' }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(170px,1fr))', gap:8, marginBottom:24 }}>
          {[
            { label:'Active clients',  value:activeCount,                        color:'#22C55E', icon:Ic.users  },
            { label:'Pending invites', value:invites.length,                     color:'#FFB547', icon:Ic.arrow  },
            { label:'Monthly revenue', value:`$${totalMRR.toLocaleString()}`,    color:'#00AAFF', icon:Ic.dollar },
            ...
            <div key={s.label} className="nexa-card" style={{ padding:'14px 18px', borderRadius:14, display:'flex', alignItems:'center', gap:11 }}>
              ...
              <div>
                ...
                <div className="nexa-label" style={{ fontFamily:"'Geist', -apple-system, sans-serif", fontSize:9, fontWeight:600, letterSpacing:'0.1em', textTransform:'uppercase' as const, color:'rgba(255,255,255,0.38)', marginTop:3 }}>{s.label}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {clients.length > 0 ? (
                ...
                <div key={c.id}
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                      ...
                      <div>
                        <div style={{ fontFamily:"'Geist', -apple-system, sans-serif", fontWeight:600, fontSize:14, color:'rgba(255,255,255,0.9)', letterSpacing:'-0.02em', marginBottom:2 }}>{c.client_name}</div>
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    ...
                    {c.client_workspace_id && (
                      <button
                        ...
                        style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 13px', borderRadius:9, fontSize:11, fontWeight:700, background:'rgba(0,170,255,0.10)', border:'1px solid rgba(77,159,255,0.22)', color:'#00AAFF', cursor:'pointer', fontFamily:"'Geist', -apple-system, sans-serif", transition:'all 0.15s', letterSpacing:'-0.01em' }}
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'44vh', textAlign:'center', padding:'40px', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:18 }}>
            ...
            <h3 style={{ fontFamily:"'Geist', -apple-system, sans-serif", fontWeight:700, fontSize:20, letterSpacing:'-0.02em', color:'rgba(255,255,255,0.85)', marginBottom:9 }}>No clients yet</h3>
            <p style={{ fontFamily:"'Geist', -apple-system, sans-serif", fontSize:14, lineHeight:1.75, color:'rgba(255,255,255,0.32)', maxWidth:360, marginBottom:22 }}>
              Add your first client to start managing their brand, content, and workspace from one place.
            </p>
            <button onClick={() => setShowNew(true)}
              style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 26px', fontFamily:"'Geist', -apple-system, sans-serif", fontWeight:700, fontSize:13, letterSpacing:'-0.02em', background:'#00AAFF', color:'#0C0C0C', border:'none', borderRadius:11, cursor:'pointer', boxShadow:'0 4px 22px rgba(77,159,255,0.38)', transition:'all 0.18s' }}>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.72)', backdropFilter:'blur(14px)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
          ...
          <div style={{ background:'rgba(10,10,18,0.98)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:20, padding:'26px', width:'100%', maxWidth:440, boxShadow:'0 32px 80px rgba(0,0,0,0.85)', animation:'pageUp 0.22s ease both' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
              <div>
                <div style={{ fontFamily:"'Geist', -apple-system, sans-serif", fontSize:17, fontWeight:800, letterSpacing:'-0.03em', color:'rgba(255,255,255,0.92)', lineHeight:1, marginBottom:3 }}>
                  Add new client
                </div>
            ...
            {[
              ...
              <div key={f.label} style={{ marginBottom:14 }}>
                ...
                <input
                  ...
                  placeholder={f.ph}
                  style={{ width:'100%', padding:'11px 13px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10, color:'rgba(255,255,255,0.88)', fontSize:13, fontFamily:"'Geist', -apple-system, sans-serif", outline:'none', transition:'border-color 0.15s', boxSizing:'border-box' as const }}
            ...
            <button onClick={addClient} disabled={!cName.trim()||creating}
              style={{ width:'100%', padding:'14px', fontSize:14, fontWeight:700, fontFamily:"'Geist', -apple-system, sans-serif", letterSpacing:'-0.02em', background:cName.trim()?'#4D9FFF':'rgba(255,255,255,0.04)', color:cName.trim()?'#000':'rgba(255,255,255,0.2)', border:'none', borderRadius:12, cursor:cName.trim()?'pointer':'not-allowed', display:'flex', alignItems:'center', justifyContent:'center', gap:9, marginTop:8, transition:'all 0.18s', boxShadow:cName.trim()?'0 4px 22px rgba(77,159,255,0.38)':'none' }}>
              {creating
