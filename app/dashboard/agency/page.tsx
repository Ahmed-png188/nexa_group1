...
function Avatar({ name, size=40 }: { name:string; size?:number }) {
  ...
  const colors = ['#00AAFF','#00AAFF','#00AAFF','#00AAFF','#00AAFF','#00AAFF','#00AAFF','#00AAFF']
  const color = '#00AAFF'
  return (
    <div style={{ width:size, height:size, borderRadius:Math.round(size*0.3), background:'rgba(0,170,255,0.12)', border:'1px solid rgba(0,170,255,0.25)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Geist', sans-serif", fontSize:Math.round(size*0.38), fontWeight:800, color:'#fff', flexShrink:0, boxShadow:'none' }}>
      {initials}
    </div>
  )
}
    ...
    <div style={{ height:'calc(100vh - var(--topbar-h))', overflowY:'auto', background:'#000' }}>
      <div style={{ position:'relative', overflow:'hidden', padding:'64px 40px', textAlign:'center' }}>
        ...
        <div style={{ position:'relative', zIndex:1 }}>
          <div style={{ width:72, height:72, borderRadius:'12px', background:'rgba(77,159,255,0.08)', border:'1px solid rgba(77,159,255,0.2)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 24px', color:'#4D9FFF' }}>
            {Ic.users}
          </div>
          <h1 style={{ fontFamily:"'Geist', sans-serif", fontSize:30, fontWeight:800, letterSpacing:'-0.05em', color:'#F4F0FF', marginBottom:12, lineHeight:1.1 }}>
          ...
          <Link href="/dashboard/settings?tab=billing"
            style={{ display:'inline-flex', alignItems:'center', gap:9, padding:'14px 32px', fontSize:15, fontWeight:700, fontFamily:"'Geist', sans-serif", letterSpacing:'-0.02em', background:'#FFFFFF', color:'#000', borderRadius:13, textDecoration:'none', boxShadow:'0 4px 28px rgba(77,159,255,0.4)', transition:'all 0.18s' }}
    ...
    <div style={{ padding:'28px 32px 48px', height:'calc(100vh - var(--topbar-h))', overflowY:'auto', background:'#000' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:28 }}>
        ...
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <h1 style={{ fontFamily:"'Geist', sans-serif", fontSize:22, fontWeight:800, letterSpacing:'-0.04em', color:'rgba(255,255,255,0.92)', lineHeight:1 }}>
              {sel.client_name}
            </h1>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:24 }}>
        {[
          ...
          <div key={s.label}  style={{ padding:'14px 16px', borderRadius:13 }}>
            <div  style={{ marginBottom:6 }}>{s.label}</div>
      ...
      {sel.client_workspace_id && (
        <div style={{ padding:'20px 24px', background:'linear-gradient(145deg, rgba(77,159,255,0.08) 0%, rgba(77,159,255,0.03) 100%)', border:'1px solid rgba(77,159,255,0.2)', borderRadius:16, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <button onClick={() => switchWs(sel.client_workspace_id)}
            style={{ display:'flex', alignItems:'center', gap:8, padding:'11px 22px', fontSize:13, fontWeight:700, fontFamily:"'Geist', sans-serif", letterSpacing:'-0.02em', background:'#FFFFFF', color:'#000', border:'none', borderRadius:11, cursor:'pointer', boxShadow:'0 4px 18px rgba(77,159,255,0.35)', transition:'all 0.18s', flexShrink:0 }}>
    ...
    <>
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28 }}>
          <div>
            <h1 style={{ fontFamily:"'Geist', sans-serif", fontSize:22, fontWeight:800, letterSpacing:'-0.03em', color:'rgba(255,255,255,0.92)', lineHeight:1, marginBottom:5 }}>Agency</h1>
            <p style={{ fontSize:12, color:'rgba(255,255,255,0.3)' }}>
              {activeCount} active client{activeCount!==1?'s':''} · ${totalMRR.toLocaleString()} MRR
            </p>
          </div>
          <button onClick={() => setShowNew(true)}
            style={{ display:'flex', alignItems:'center', gap:8, padding:'9px 18px', fontFamily:"'Geist', sans-serif", fontWeight:700, fontSize:13, letterSpacing:'-0.02em', background:'#FFFFFF', color:'#000', border:'none', borderRadius:11, cursor:'pointer', transition:'background 0.15s' }}
            onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background='rgba(255,255,255,0.88)';(e.currentTarget as HTMLElement).style.background='rgba(255,255,255,0.88)'}}
            onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background='#FFFFFF';(e.currentTarget as HTMLElement).style.boxShadow='0 4px 18px rgba(77,159,255,0.35)'}}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:24 }}>
          {[
            ...
            <div key={s.label}  style={{ padding:'14px 18px', borderRadius:14, display:'flex', alignItems:'center', gap:11 }}>
              ...
              <div>
                <div  style={{ fontFamily:"'Geist Mono', monospace", fontWeight:300, fontSize:28, letterSpacing:'-0.04em', color:s.color }}>{s.value}</div>
                <div  style={{ fontFamily:"'Geist', sans-serif", fontSize:9, fontWeight:600, letterSpacing:'0.1em', textTransform:'uppercase' as const, color:'var(--t4)', marginTop:3 }}>{s.label}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {clients.length > 0 ? (
                ...
                <div key={c.id}
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                      ...
                      <div>
                        <div style={{ fontFamily:"'Geist', sans-serif", fontWeight:600, fontSize:14, color:'rgba(255,255,255,0.9)', letterSpacing:'-0.02em', marginBottom:2 }}>{c.client_name}</div>
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    ...
                    {c.client_workspace_id && (
                      <button
                        ...
                        style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 13px', borderRadius:9, fontSize:11, fontWeight:700, background:'rgba(77,159,255,0.1)', border:'1px solid rgba(77,159,255,0.22)', color:'#4D9FFF', cursor:'pointer', fontFamily:"'Geist', sans-serif", transition:'all 0.15s', letterSpacing:'-0.01em' }}
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'44vh', textAlign:'center', padding:'40px', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:'10px' }}>
            ...
            <h3 style={{ fontFamily:"'Geist', sans-serif", fontWeight:700, fontSize:20, letterSpacing:'-0.02em', color:'rgba(255,255,255,0.85)', marginBottom:9 }}>No clients yet</h3>
            <p style={{ fontFamily:"'Geist', sans-serif", fontSize:14, lineHeight:1.75, color:'rgba(255,255,255,0.32)', maxWidth:360, marginBottom:22 }}>
              Add your first client to start managing their brand, content, and workspace from one place.
            </p>
            <button onClick={() => setShowNew(true)}
              style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 26px', fontFamily:"'Geist', sans-serif", fontWeight:700, fontSize:13, letterSpacing:'-0.02em', background:'#FFFFFF', color:'#000', border:'none', borderRadius:11, cursor:'pointer', boxShadow:'0 4px 22px rgba(77,159,255,0.38)', transition:'all 0.18s' }}>
        ...
        {invites.length > 0 && (
          <div style={{ marginTop:28 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
              {invites.map(inv => (
                <div key={inv.id} style={{ display:'flex', alignItems:'center', gap:13, padding:'13px 16px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:13 }}>
                  ...
                  <span style={{ fontSize:10, fontWeight:700, padding:'3px 10px', borderRadius:100, background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.18)', color:'#F59E0B', textTransform:'uppercase', letterSpacing:'0.05em' }}>
                    Pending
                  </span>
                </div>
              ))}
      ...
      {showNew && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.72)', backdropFilter:'blur(14px)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
          ...
          <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'12px', padding:'26px', width:'100%', maxWidth:440, boxShadow:'0 32px 80px rgba(0,0,0,0.85)', animation:'pageUp 0.22s ease both' }}>

            {/* Header */}
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
              <div>
                <div style={{ fontFamily:"'Geist', sans-serif", fontSize:17, fontWeight:800, letterSpacing:'-0.03em', color:'rgba(255,255,255,0.92)', lineHeight:1, marginBottom:3 }}>
                  Add new client
                </div>
            ...
            {[
              ...
              <div key={f.label} style={{ marginBottom:14 }}>
                ...
                <input
                  ...
                  placeholder={f.ph}
                  style={{ width:'100%', padding:'11px 13px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10, color:'rgba(255,255,255,0.88)', fontSize:13, fontFamily:"'Geist', sans-serif", outline:'none', transition:'border-color 0.15s', boxSizing:'border-box' as const }}
            ...
            <button onClick={addClient} disabled={!cName.trim()||creating}
              style={{ width:'100%', padding:'14px', fontSize:14, fontWeight:700, fontFamily:"'Geist', sans-serif", letterSpacing:'-0.02em', background:cName.trim()?'#4D9FFF':'rgba(255,255,255,0.04)', color:cName.trim()?'#0C0C0C':'rgba(255,255,255,0.2)', border:'none', borderRadius:12, cursor:cName.trim()?'pointer':'not-allowed', display:'flex', alignItems:'center', justifyContent:'center', gap:9, marginTop:8, transition:'all 0.18s', boxShadow:cName.trim()?'0 4px 22px rgba(77,159,255,0.38)':'none' }}>
              {creating
