...
import { brandBrain as bbCopy, t } from '@/content/copy'
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'6px', marginTop:value?'10px':'0' }}>
          {tags.map((tag,i) => (
            <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:'8px' }}>
              ...
              <span style={{ fontSize:'12px', color:'rgba(255,255,255,0.65)', lineHeight:1.55 }}>{tag}</span>
            </div>
          ))}
        </div>
      )}
...
// ─── Trait Bar ────────────────────────────────────────────────
function TraitBar({ label, value, isArabic }: { label:string; value:number; isArabic:boolean }) {
  const levelLabel = isArabic
    ? (value < 33 ? 'منخفض' : value < 66 ? 'متوسط' : 'مرتفع')
  ...
  return (
    <div style={{ marginBottom:'16px' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'8px' }}>
        ...
        <span style={{ fontSize:'11px', fontWeight:600, color:'rgba(255,255,255,0.40)' }}>{levelLabel}</span>
      </div>
...
export default function BrandPage() {
  ...
  const c                = t(bbCopy, lang)

  const TABS = [
    { id:'overview' as Tab, label: isArabic ? 'نظرة عامة' : 'Overview', icon:Ic.brain },
    { id:'voice'    as Tab, label: isArabic ? 'الصوت'      : 'Voice',    icon:Ic.mic   },
    { id:'audience' as Tab, label: isArabic ? 'الجمهور'    : 'Audience', icon:Ic.users },
    { id:'visual'   as Tab, label: isArabic ? 'الهوية البصرية' : 'Visual', icon:Ic.eye  },
    { id:'assets'   as Tab, label: isArabic ? 'الملفات'    : 'Assets',   icon:Ic.file  },
  ]

  const ASSET_TYPES = isArabic ? [
    { id:'logo',          label:'شعار'        },
    { id:'sample_post',   label:'منشور نموذجي' },
    { id:'product_photo', label:'صورة منتج'   },
    { id:'brand_doc',     label:'وثيقة براند' },
    { id:'other',         label:'أخرى'        },
  ] : [
    { id:'logo',          label:'Logo'          },
  ...
  async function analyze() {
    ...
    try {
      ...
      const r = await fetch('/api/analyze-brand',{ method:'POST', credentials:'include', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ workspace_id:ws.id, files:filePayloads, brand_name:ws.brand_name||ws.name, website_url:ws.brand_website, lang }) })
      const d = await r.json()
      if (d.success) {
        setProfile(d.profile)
        showToast(isArabic ? 'Brand Brain تحدّثت — كل ما تنتجه Nexa صار أحسن' : 'Brand Brain updated — every output now sounds sharper')
      } else {
        showToast(d.error || (isArabic ? 'فشل التحليل — جرّب مرة ثانية' : 'Analysis failed — try again'), false)
      }
    } catch {
      showToast(isArabic ? 'صار خطأ ما' : 'Something went wrong', false)
    }
    setAnalyzing(false)
  }
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      if (error) { showToast(isArabic ? 'فشل الرفع — تحقق من الملف وجرّب مرة ثانية' : 'Upload failed — check your file and try again', false); return }
      ...
      showToast(isArabic ? 'تم الرفع — أضف المزيد أو شغّلي التحليل' : 'Asset uploaded — add more or run analysis')
      load()
    } catch {
      showToast(isArabic ? 'فشل الرفع — تحقق من الملف وجرّب مرة ثانية' : 'Upload failed — check your file and try again', false)
    }
    setUploading(false)
  }
  ...
  async function del(id: string) {
    ...
    showToast(isArabic ? 'تم الحذف' : 'Removed'); load()
  }
  ...
  const voiceTraits = isArabic ? [
    { label:'المباشرة',   value: voice.directness   || voiceScore        },
    { label:'الثقة',     value: voice.authority    || audienceScore      },
    { label:'الدفء',     value: voice.warmth       || 62                 },
    { label:'الجرأة',    value: voice.provocation  || Math.round(voiceScore * 0.82) },
    { label:'السرد',     value: voice.storytelling || Math.round(voiceScore * 0.65) },
  ] : [
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'calc(100vh - var(--topbar-h))', flexDirection:'column', gap:'14px', background:'#0C0C0C' }}>
      ...
      <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.25)', letterSpacing:'0.04em' }}>{isArabic ? 'لحظة...' : 'Loading'}</div>
    </div>
  )

  // ── Untrained State ────────────────────────────────────────
  if (!profile && tab !== 'assets') return (
    <div style={{ height:'calc(100vh - var(--topbar-h))', overflowY:'auto', backgroundImage:'url(/cyan-header.png)', backgroundSize:'cover', backgroundPosition:'center', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 32px', position:'relative' }}>
      ...
      <div style={{ position:'relative', zIndex:1, textAlign:'center', maxWidth:620, animation:'pageUp 0.4s cubic-bezier(0.22,1,0.36,1) both' }}>
        ...
        <div style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'6px 14px', borderRadius:999, background:'rgba(0,0,0,0.12)', border:'1px solid rgba(0,0,0,0.16)', marginBottom:32, backdropFilter:'blur(8px)' }}>
          ...
          <span style={{ fontSize:12, fontWeight:600, color:'rgba(0,0,0,0.65)', letterSpacing:'0.02em' }}>
            {isArabic ? 'Brand Brain · لم تتدرّب بعد' : 'Brand Brain · Not trained yet'}
          </span>
        </div>

        {/* Headline */}
        <h1 style={{ fontSize:'clamp(32px,5vw,52px)', fontWeight:800, letterSpacing:'-0.04em', color:'#0C0C0C', lineHeight:1.1, marginBottom:16 }}>
          {isArabic ? <>Brand Brain تنتظرك.<br/>أمدّها بما تحتاج.</> : <>Your Brand Brain<br/>is waiting.</>}
        </h1>

        {/* Subline */}
        <p style={{ fontSize:16, color:'rgba(0,0,0,0.52)', lineHeight:1.75, maxWidth:460, margin:'0 auto 40px' }}>
          {isArabic
            ? 'ارفع كونتنتك وشعاراتك ووثائق براندك. Nexa تقرأ كل شيء وتبني ملفاً يجعل كل ما تنتجه يبدو وكأنك كتبته بنفسك.'
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:40 }}>
          {(isArabic ? [
            { icon:Ic.mic,   title:'صوتك محدّد',   desc:'كل ما تنتجه Nexa يبدو وكأنك أنت كتبته' },
            { icon:Ic.users, title:'جمهورك معروف',  desc:'تكتب مباشرة لعملائك، مو للإنترنت كله' },
            { icon:Ic.eye,   title:'هويتك البصرية', desc:'توجّه كل صورة تنتجها Nexa بأسلوبك' },
          ] : [
            ...
            <div key={i} style={{ padding:'20px 18px', borderRadius:14, background:'rgba(255,255,255,0.72)', border:'1px solid rgba(255,255,255,0.90)', backdropFilter:'blur(16px)', textAlign: isArabic ? 'right' : 'left' }}>
        ...
        <div style={{ display:'flex', gap:10, justifyContent:'center', alignItems:'center' }}>
          ...
          {assets.length > 0 && (
            <button onClick={analyze} disabled={analyzing}
              style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'13px 24px', background:'rgba(0,0,0,0.12)', border:'1px solid rgba(0,0,0,0.18)', borderRadius:12, fontSize:14, fontWeight:600, color:'rgba(0,0,0,0.72)', cursor:analyzing?'not-allowed':'pointer', transition:'all 0.15s', backdropFilter:'blur(8px)' }}>
              {analyzing
                ? <><div className="nexa-spinner" style={{ width:13, height:13 }}/>{isArabic ? 'Nexa تحلّل...' : 'Analyzing…'}</>
                : <><span style={{ display:'flex' }}>{Ic.bolt}</span>{isArabic ? `حلّلي ${assets.length} ملف` : `Analyze ${assets.length} asset${assets.length!==1?'s':''}`}</>}
            </button>
          )}
        </div>
        ...
        <div style={{ marginTop:14, fontSize:12, color:'rgba(0,0,0,0.42)' }}>
          {isArabic ? 'يدعم PNG, JPG, PDF, DOCX, TXT · تقدر تضيف أكثر لاحقاً' : 'Supports PNG, JPG, PDF, DOCX, TXT · You can add more later'}
        </div>
        ...
        {assets.length > 0 && (
          <div style={{ marginTop:32 }}>
            <div style={{ fontSize:10, fontWeight:600, color:'rgba(0,0,0,0.40)', letterSpacing:'0.08em', textTransform:'uppercase' as const, marginBottom:12 }}>
              {isArabic ? `${assets.length} ملف مرفوع` : `${assets.length} asset${assets.length!==1?'s':''} uploaded`}
            </div>
  ...
  // ── Trained State ──────────────────────────────────────────
  return (
    <div style={{ height:'calc(100vh - var(--topbar-h))', overflowY:'auto', background:'#0C0C0C' }}>
      ...
      <div style={{ opacity:mounted?1:0, transition:'opacity 0.4s ease' }}>
        <div style={{ backgroundImage:'url(/cyan-header.png)', backgroundSize:'cover', backgroundPosition:'center top', padding:'40px 0 28px' }}>
          <div style={{ padding:'0 36px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:'20px' }}>
            <div>
              <h1 style={{ fontSize:'36px', fontWeight:700, letterSpacing:'-0.04em', color:'#0A0A0A', lineHeight:1, marginBottom:'8px' }}>{c.heading}</h1>
              <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                ...
                <span style={{ fontSize:'13px', color:'rgba(0,0,0,0.60)', fontWeight:500 }}>
                  {isArabic
                    ? `نشطة · ${assets.length} ملف · ${learnings.length} تعلّم`
            ...
            <button onClick={analyze} disabled={analyzing}
              ...
              {analyzing
                ? <><div className="nexa-spinner" style={{ width:12, height:12 }}/>{isArabic ? 'Nexa تحلّل...' : 'Analyzing…'}</>
                : <><span style={{ display:'flex' }}>{Ic.refresh}</span>{c.reanalyze}</>}
            </button>
          </div>
        </div>
            ...
            {TABS.map(tabItem => {
              ...
              return (
                <button key={tabItem.id} onClick={() => setTab(tabItem.id)}
                  style={{ display:'flex', alignItems:'center', gap:'7px', padding:'12px 20px', background:'none', border:'none', borderBottom:`2px solid ${active ? '#00AAFF' : 'transparent'}`, marginBottom:'-1px', color: active ? '#FFFFFF' : 'rgba(255,255,255,0.42)', cursor:'pointer', fontSize:'13px', fontWeight: active ? 600 : 400, transition:'all 0.15s', whiteSpace:'nowrap' }}
      ...
      <div style={{ padding:'28px 36px 48px' }}>
          ...
          <div className="b-in">
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'28px' }}>
                <div>
                  <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'4px' }}>
                    {isArabic ? 'صحة البراند' : 'Brand Health'}
                  </div>
                  <div style={{ fontSize:'13px', color:'rgba(255,255,255,0.55)' }}>
                    {isArabic ? 'كم تعرف Nexa براندك' : 'How well Nexa knows your brand'}
                  </div>
                </div>
                <div style={{ textAlign:'center' }}>
                  ...
                  <div style={{ fontSize:'10px', color:'rgba(255,255,255,0.35)', marginTop:'4px', letterSpacing:'0.04em', textTransform:'uppercase' }}>
                    {isArabic ? 'الإجمالي' : 'Overall'}
                  </div>
                </div>
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'20px' }}>
                <ScoreRing score={voiceScore}    color="#00AAFF" label={c.bi_voiceMatch}  size={88}/>
                <ScoreRing score={audienceScore} color="#22C55E" label={c.bi_audienceFit} size={88}/>
                <ScoreRing score={visualScore}   color="#F59E0B" label={c.bi_visualStyle} size={88}/>
              </div>
            </div>
            ...
            {profile?.brand_summary && (
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px 24px', marginBottom:'12px' }}>
                <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'10px' }}>
                  {isArabic ? 'ملخص البراند' : 'Brand Summary'}
                </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'12px' }}>
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px' }}>
                <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'14px' }}>
                  {isArabic ? 'بصمة الصوت' : 'Voice Fingerprint'}
                </div>
                {[
                  { l: isArabic ? 'النبرة'      : 'Tone',        v:voice.tone||ws?.brand_voice||'—' },
                  { l: isArabic ? 'الأسلوب'     : 'Style',       v:voice.style||'—'                 },
                  { l: isArabic ? 'الشخصية'     : 'Personality', v:voice.personality||'—'           },
                ].map(item => item.v !== '—' && (
              ...
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px' }}>
                <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'14px' }}>
                  {isArabic ? 'ذكاء الجمهور' : 'Audience Intelligence'}
                </div>
                {[
                  { l: isArabic ? 'الملف'  : 'Profile', v:aud.primary||aud.demographics||'—'                                     },
                  { l: isArabic ? 'الألم'  : 'Pain',    v:Array.isArray(aud.pain_points)?aud.pain_points[0]:aud.pain_points||'—' },
                  { l: isArabic ? 'الهدف'  : 'Goal',    v:Array.isArray(aud.goals)?aud.goals[0]:aud.goals||'—'                  },
                ].map(item => item.v !== '—' && (
                  <div key={item.l} style={{ display:'flex', gap:'10px', marginBottom:'10px' }}>
                    <span style={{ fontSize:'11px', color:'rgba(255,255,255,0.28)', width:'72px', flexShrink:0, paddingTop:'1px' }}>{item.l}</span>
            ...
            <div style={{ marginBottom:'12px' }}>
              <button onClick={() => { const params = new URLSearchParams({ view:'compose', context:`Writing to my audience: ${aud?.primary||aud?.demographics||ws?.brand_audience||'my target audience'}`, objective:'nurture' }); window.location.href = `/dashboard/automate?${params.toString()}` }}
                style={{ display:'inline-flex', alignItems:'center', gap:'7px', padding:'9px 18px', background:'transparent', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', fontSize:'12px', fontWeight:500, color:'rgba(255,255,255,0.50)', cursor:'pointer', transition:'all 0.15s' }}
                ...
                <span style={{ display:'flex' }}>{Ic.email}</span>
                {isArabic ? 'راسل جمهورك' : 'Email my audience'}
                <span style={{ display:'flex' }}>{Ic.arrow}</span>
              </button>
            </div>
              ...
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px 24px' }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px' }}>
                  <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)' }}>
                    {isArabic ? 'آخر ما تعلّمته Nexa' : 'Recent Learnings'}
                  </div>
                  <span style={{ fontSize:'11px', color:'rgba(255,255,255,0.25)' }}>
                    {isArabic ? `${learnings.length} إجمالاً` : `${learnings.length} total`}
                  </span>
                </div>
          ...
          <div className="b-in">
            <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'24px', marginBottom:'12px' }}>
              <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'20px' }}>
                {isArabic ? 'خصائص الصوت' : 'Voice Characteristics'}
              </div>
              {voiceTraits.map(tr => <TraitBar key={tr.label} label={tr.label} value={tr.value} isArabic={isArabic}/>)}
            </div>

            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'12px' }}>
              <InfoBlock label={isArabic ? 'نبرة البراند'     : 'Brand tone'}         value={voice.primary_tone||voice.tone||ws?.brand_voice}/>
              <InfoBlock label={isArabic ? 'أسلوب الكتابة'    : 'Writing style'}      value={voice.writing_style||voice.style}/>
              <InfoBlock label={isArabic ? 'سمات الشخصية'     : 'Personality traits'} value={voice.personality||voice.sentence_structure}/>
              <InfoBlock label={isArabic ? 'مقاربة الكونتنت'  : 'Content approach'}   value={voice.content_approach||voice.cta_style}/>
              <InfoBlock label={isArabic ? 'العبارات المميزة'  : 'Signature phrases'}  tags={voice.key_phrases||voice.vocabulary||[]}/>
              <InfoBlock label={isArabic ? 'تجنّب هذا'         : 'Topics to avoid'}    tags={voice.avoid||voice.forbidden||[]}/>
            </div>

            {voice.examples && voice.examples.length > 0 && (
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px 24px' }}>
                <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'14px' }}>
                  {isArabic ? 'كيف يبدو صوت براندك' : 'How Your Voice Sounds'}
                </div>
                <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                  {voice.examples.slice(0,3).map((ex: string, i: number) => (
                    <div key={i} style={{ padding:'14px 18px', background:'rgba(0,170,255,0.04)', border:'1px solid rgba(0,170,255,0.12)', borderLeft: isArabic ? 'none' : '3px solid rgba(0,170,255,0.40)', borderRight: isArabic ? '3px solid rgba(0,170,255,0.40)' : 'none', borderRadius: isArabic ? '8px 0 0 8px' : '0 8px 8px 0' }}>
            ...
            {!Object.keys(voice).length && !ws?.brand_voice && (
              <div style={{ textAlign:'center', padding:'48px', color:'rgba(255,255,255,0.25)', fontSize:'13px', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:'10px' }}>
                {isArabic ? 'ارفع كونتنت براندك وشغّلي التحليل لاستخراج ملف صوتك.' : 'Upload brand content and run analysis to extract your voice profile.'}
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'audience' && (
          <div className="b-in">
            {(aud.primary||aud.demographics) && (
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'24px', marginBottom:'12px' }}>
                <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'10px' }}>
                  {isArabic ? 'مع مين تتكلم' : "Who You're Talking To"}
                </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'12px', marginBottom:'12px' }}>
              <InfoBlock label={isArabic ? 'الجمهور الأساسي'   : 'Primary audience'}   value={aud.primary||aud.demographics||''}/>
              <InfoBlock label={isArabic ? 'علم النفس'          : 'Psychographics'}     value={aud.psychographics||aud.psychology||''}/>
              <InfoBlock label={isArabic ? 'نقاط الألم'         : 'Pain points'}        tags={Array.isArray(aud.pain_points)?aud.pain_points:[aud.pain_points].filter(Boolean)}/>
              <InfoBlock label={isArabic ? 'الأهداف والرغبات'   : 'Goals & desires'}    tags={Array.isArray(aud.goals)?aud.goals:[aud.goals].filter(Boolean)}/>
              <InfoBlock label={isArabic ? 'الكونتنت اللي يحبونه' : 'Content they love'}  value={aud.content_preferences}/>
              <InfoBlock label={isArabic ? 'المنصات اللي يستخدمونها' : 'Platforms they use'} tags={Array.isArray(aud.platforms)?aud.platforms:[]}/>
            </div>
            ...
            {aud.engagement_triggers && (
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px 24px' }}>
                <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'12px' }}>
                  {isArabic ? 'إيش يجعلهم يتفاعلون' : 'What Makes Them Engage'}
                </div>
        ...
        {tab === 'visual' && (
          <div className="b-in">
            {(visual.colors||visual.color_palette) && (
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'24px', marginBottom:'12px' }}>
                ...
                <div style={{ display:'flex', gap:'10px', flexWrap:'wrap', marginBottom:'12px' }}>
                  {([...(visual.colors||[]), visual.color_palette, visual.color_mood].filter(Boolean)).map((clr: string, i: number) => {
                    const isHex = clr.startsWith('#')
                    return (
                      <div key={i} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'6px' }}>
                        {isHex && <div style={{ width:44, height:44, borderRadius:'10px', background:clr, border:'1px solid rgba(255,255,255,0.10)' }}/>}
                        <span style={{ fontSize:'10px', color:'rgba(255,255,255,0.40)', fontFamily:"'Geist Mono', monospace" }}>{clr}</span>
                      </div>
                    )
                  })}
                </div>
                {!visual.colors?.some((clr: string) => clr.startsWith('#')) && (
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'12px', marginBottom:'12px' }}>
              <InfoBlock label={isArabic ? 'الأسلوب البصري'    : 'Visual style'}    value={visual.aesthetic||visual.style||''}/>
              <InfoBlock label={isArabic ? 'الخطوط'            : 'Typography'}      value={visual.typography||visual.fonts?.join(', ')||''}/>
              <InfoBlock label={isArabic ? 'توجيه الصور'       : 'Image direction'} value={visual.image_guidelines||visual.photography_style||''}/>
              <InfoBlock label={isArabic ? 'كلمات الهوية البصرية' : 'Visual keywords'} tags={visual.keywords||visual.video_style?[visual.video_style]:[]}/>
            </div>

            {assets.filter((a:any) => a.type==='logo'||a.type==='product_photo').length > 0 && (
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px 24px' }}>
                <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'14px' }}>
                  {isArabic ? 'مراجع بصرية' : 'Visual References'}
                </div>
        ...
        {tab === 'assets' && (
          <div className="b-in">
            ...
            <div
              ...
              onClick={() => fileRef.current?.click()}
              style={{ padding:'40px 32px', border:`1px dashed ${dragOver?'rgba(0,170,255,0.40)':'rgba(255,255,255,0.10)'}`, borderRadius:'10px', textAlign:'center', cursor:'pointer', background:dragOver?'rgba(0,170,255,0.04)':'rgba(255,255,255,0.02)', transition:'all 0.15s', marginBottom:'24px' }}>
              ...
              <div style={{ fontSize:'15px', fontWeight:600, color:'rgba(255,255,255,0.72)', marginBottom:'6px', letterSpacing:'-0.01em' }}>
                {uploading
                  ? (isArabic ? 'Nexa ترفع...' : 'Uploading…')
                  : dragOver
                  ? (isArabic ? 'ارمِه هنا' : 'Drop it here')
                  : c.upload_label}
              </div>
              <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.30)', lineHeight:1.65 }}>
                {isArabic
                  ? 'شعارات، منشورات نموذجية، صور منتجات، وثائق براند.\nكلما أعطيت Nexa أكثر، كلما عرفتك أعمق.'
                  : 'Logos, sample posts, product photos, brand guidelines, tone docs.\nThe richer the input, the sharper the intelligence.'}
              </div>
              <input ref={fileRef} type="file" accept="image/*,.pdf,.doc,.docx,.txt" style={{ display:'none' }} onChange={e => e.target.files?.[0] && upload(e.target.files[0])}/>
            </div>
            ...
            <div style={{ marginBottom:'24px' }}>
              <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)', marginBottom:'12px' }}>
                {isArabic ? 'ترفع كـ' : 'Uploading as'}
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:'8px' }}>
                {ASSET_TYPES.map(at => (
                  <button key={at.id} onClick={() => setAssetType(at.id)}
                    style={{ padding:'9px 8px', borderRadius:'8px', fontSize:'11px', fontWeight:500, background: assetType===at.id ? 'rgba(0,170,255,0.10)' : 'rgba(255,255,255,0.02)', border:`1px solid ${assetType===at.id ? 'rgba(0,170,255,0.25)' : 'rgba(255,255,255,0.08)'}`, color: assetType===at.id ? '#00AAFF' : 'rgba(255,255,255,0.35)', cursor:'pointer', transition:'all 0.14s', textAlign:'center', whiteSpace:'nowrap' }}>
                    {at.label}
                  </button>
                ))}
              </div>
              ...
              <>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'14px' }}>
                  ...
                  {assets.length >= 2 && !profile && (
                    <button onClick={analyze} disabled={analyzing}
                      style={{ display:'flex', alignItems:'center', gap:'6px', padding:'7px 16px', fontSize:'12px', fontWeight:600, background:'#FFFFFF', color:'#0C0C0C', border:'none', borderRadius:'8px', cursor:'pointer', transition:'background 0.15s' }}
                      onMouseEnter={e => (e.currentTarget as HTMLElement).style.background='rgba(255,255,255,0.88)'}
                      onMouseLeave={e => (e.currentTarget as HTMLElement).style.background='#FFFFFF'}>
                      {analyzing
                        ? (isArabic ? 'Nexa تحلّل...' : 'Analyzing…')
                        : <><span style={{ display:'flex' }}>{Ic.bolt}</span>{isArabic ? 'حلّلي الآن' : 'Analyze now'}</>}
                    </button>
                  )}
                </div>
                ...
                {profile && (
                  <div style={{ marginTop:'20px', padding:'16px 20px', background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.45)', lineHeight:1.5 }}>
                      {isArabic ? 'أضفت ملفات جديدة؟ أعيدي التحليل وخلّ Nexa تتحدّث.' : 'Added new assets? Re-analyze to keep Brand Brain sharp.'}
                    </div>
                    <button onClick={analyze} disabled={analyzing}
                      style={{ display:'flex', alignItems:'center', gap:'6px', padding:'7px 16px', fontSize:'12px', fontWeight:600, background:'#FFFFFF', color:'#0C0C0C', border:'none', borderRadius:'8px', cursor:'pointer', flexShrink:0, marginLeft:'16px', transition:'background 0.15s' }}
                      onMouseEnter={e => (e.currentTarget as HTMLElement).style.background='rgba(255,255,255,0.88)'}
                      onMouseLeave={e => (e.currentTarget as HTMLElement).style.background='#FFFFFF'}>
                      <span style={{ display:'flex' }}>{Ic.bolt}</span>
                      {analyzing ? (isArabic ? 'Nexa تحلّل...' : 'Analyzing…') : c.reanalyze}
                    </button>
                  </div>
                )}
              </>
            ...
            {assets.length === 0 && (
              <div style={{ textAlign:'center', padding:'40px', color:'rgba(255,255,255,0.25)', fontSize:'13px', lineHeight:1.7 }}>
                {isArabic
                  ? <>ما في ملفات بعد.<br/><span style={{ color:'rgba(255,255,255,0.18)', fontSize:'12px' }}>كلما أعطيت Nexa أكثر، كلما عرفتك أعمق.</span></>
                  : <>No assets uploaded yet.<br/><span style={{ color:'rgba(255,255,255,0.18)', fontSize:'12px' }}>The more you give Nexa, the better it knows you.</span></>}
              </div>
            )}
          </div>
        )}
