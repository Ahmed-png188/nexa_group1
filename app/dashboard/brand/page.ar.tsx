...
type Tab = 'نظرة عامة' | 'الصوت' | 'الجمهور' | 'الهوية البصرية' | 'الملفات'
...
function TraitBar({ label, value }: { label: string; value: number }) {
  const levelLabel = value < 33 ? 'منخفض' : value < 66 ? 'متوسط' : 'مرتفع'
  return (
...
// ─── الصفحة الرئيسية ──────────────────────────────────────────
export default function BrandBrainAr() {
  const supabase = createClient()
  const fileRef  = useRef<HTMLInputElement>(null)

  const TABS: Tab[] = ['نظرة عامة', 'الصوت', 'الجمهور', 'الهوية البصرية', 'الملفات']
  const TAB_ICONS: Record<Tab, React.ReactNode> = {
    'نظرة عامة':      Ic.brain,
    'الصوت':           Ic.mic,
    'الجمهور':         Ic.users,
    'الهوية البصرية':  Ic.eye,
    'الملفات':         Ic.file,
  }
  ...
  async function analyze() {
    if (!ws || analyzing) return
    setAnalyzing(true)
    try {
      const filePayloads: { name: string; type: string; base64: string }[] = []
      const imageAssets = assets
        .filter(a => a.file_url && !a.file_url.startsWith('internal://') && !a.file_url.startsWith('uploaded://'))
        .slice(0, 5)
      ...
      if (d.success) {
        ...
        showToast('Brand Brain تحدّثت — كل ما تنتجه Nexa صار أحسن')
      } else {
        showToast(d.error || 'فشل التحليل — جرّب مرة ثانية', false)
      }
    } catch {
      showToast('صار خطأ غير متوقع — حاول مجدداً', false)
    }
    setAnalyzing(false)
  }
  ...
  async function upload(file: File) {
    if (!ws || uploading) return
    setUploading(true)
    try {
      const ext  = file.name.split('.').pop()
      const path = `${ws.id}/${assetType}/${Date.now()}.${ext}`
      const { error } = await supabase.storage.from('brand-assets').upload(path, file)
      if (error) { showToast('فشل الرفع — تحقق من الملف وحاول مجدداً', false); return }
      ...
      showToast('تم الرفع — أضف المزيد أو شغّل التحليل')
      load()
    } catch {
      showToast('فشل الرفع — تحقق من الملف وحاول مجدداً', false)
    }
    setUploading(false)
  }
  ...
  // ══════════════════════════════════════════════════════════
  // الحالة الأولى — قبل التدريب
  // ══════════════════════════════════════════════════════════
  if (!profile && tab !== 'الملفات') return (
    <div
      dir="rtl"
      style={{ height: 'calc(100vh - var(--topbar-h))', overflowY: 'auto', backgroundImage: 'url(/cyan-header.png)', backgroundSize: 'cover', backgroundPosition: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 32px', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: 620, animation: 'pageUp 0.4s cubic-bezier(0.22,1,0.36,1) both' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 40 }}>
          {[
            ...
            { icon: Ic.eye,   title: 'هويتك البصرية محفوظة',  desc: 'توجّه كل صورة تنتجها Nexa بأسلوبك الخاص' },
          ].map((item, i) => (
            <div key={i} style={{ padding: '20px 18px', borderRadius: 14, background: 'rgba(255,255,255,0.72)', border: '1px solid rgba(255,255,255,0.90)', backdropFilter: 'blur(16px)', textAlign: 'right' }}>
          ...
          {assets.length > 0 && (
            <button onClick={analyze} disabled={analyzing}
              ...
              {analyzing
                ...
                : <><span style={{ display: 'flex' }}>{Ic.bolt}</span>حلّل {assets.length} {assets.length === 1 ? 'ملف' : 'ملفات'}</>}
            </button>
          )}
        </div>
        ...
        <div style={{ marginTop: 14, fontSize: 12, color: 'rgba(0,0,0,0.42)' }}>
          يدعم PNG، JPG، PDF، DOCX، TXT · تقدر تضيف أكثر لاحقاً
        </div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' as const }}>
              {assets.slice(0, 4).map(a => (
                <div key={a.id} style={{ padding: '7px 14px', background: 'rgba(255,255,255,0.65)', border: '1px solid rgba(255,255,255,0.85)', borderRadius: 8, backdropFilter: 'blur(8px)' }}>
                  ...
                  <div style={{ fontSize: 10, color: 'rgba(0,0,0,0.45)', marginTop: 2 }}>{a.type?.replace('_', ' ')}</div>
                </div>
              ))}
            </div>
          </div>
  ...
  // ══════════════════════════════════════════════════════════
  // الحالة المُدرَّبة — واجهة كاملة
  // ══════════════════════════════════════════════════════════
  return (
    <div dir="rtl" style={{ height: 'calc(100vh - var(--topbar-h))', overflowY: 'auto', background: '#0C0C0C' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontSize: '13px', color: 'rgba(0,0,0,0.60)', fontWeight: 500 }}>
                  نشطة · {assets.length} {assets.length === 1 ? 'ملف' : 'ملفات'} · {learnings.length} {learnings.length === 1 ? 'تعلّم' : 'تعلّمات'}
                </span>
              </div>
            </div>
            <button onClick={analyze} disabled={analyzing}
              ...
              {analyzing
                ? <><div className="nexa-spinner" style={{ width: 12, height: 12 }}/>Nexa تحلّل...</>
                : <><span style={{ display: 'flex' }}>{Ic.refresh}</span>حلّل من جديد</>}
            </button>
          </div>
        </div>
            ...
            {TABS.map(t => {
              ...
              return (
                <button key={t} onClick={() => setTab(t)}
                  ...
                  <span style={{ display: 'flex', opacity: active ? 1 : 0.5 }}>{TAB_ICONS[t]}</span>
                  {t}
                </button>
              )
            })}
      ...
      <div style={{ padding: '28px 36px 48px' }}>
          ...
          <div className="b-in">
            ...
            <div style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', padding: '28px 32px', marginBottom: '12px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '20px' }}>
                <ScoreRing score={voiceScore}    color="#00AAFF" label="توافق الصوت"     size={88}/>
                <ScoreRing score={audienceScore} color="#22C55E" label="ملاءمة الجمهور" size={88}/>
                <ScoreRing score={visualScore}   color="#F59E0B" label="الأسلوب البصري" size={88}/>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', padding: '20px' }}>
                ...
                {[
                  { l: 'النبرة',   v: voice.tone        || ws?.brand_voice || '—' },
                  { l: 'الأسلوب',  v: voice.style       || '—' },
                  { l: 'الشخصية',  v: voice.personality || '—' },
                ].map(item => item.v !== '—' && (
              ...
              <div style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', padding: '20px' }}>
                ...
                {[
                  { l: 'الملف',      v: aud.primary || aud.demographics || '—' },
                  { l: 'نقطة الألم', v: Array.isArray(aud.pain_points) ? aud.pain_points[0] : aud.pain_points || '—' },
                  { l: 'الهدف',      v: Array.isArray(aud.goals)       ? aud.goals[0]       : aud.goals       || '—' },
                ].map(item => item.v !== '—' && (
                  <div key={item.l} style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                    <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.28)', width: '72px', flexShrink: 0, paddingTop: '1px' }}>{item.l}</span>
                    <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.70)', lineHeight: 1.5 }}>
              ...
              <button
                onClick={() => {
                  const params = new URLSearchParams({
                    view:      'compose',
                    context:   `أكتب لجمهوري: ${aud?.primary || aud?.demographics || ws?.brand_audience || 'جمهوري المستهدف'}`,
                    objective: 'nurture',
                  })
              ...
              <div style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', padding: '20px 24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                  ...
                  <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)' }}>
                    {learnings.length} {learnings.length === 1 ? 'تعلّم' : 'إجمالاً'}
                  </span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  {learnings.slice(0, 6).map(l => (
                    <div key={l.id}
                      style={{ display: 'flex', gap: '12px', padding: '10px 12px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', transition: 'background 0.12s' }}
        ...
        {tab === 'الصوت' && (
          <div className="b-in">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <InfoBlock label="نبرة البراند"     value={voice.primary_tone     || voice.tone     || ws?.brand_voice}/>
              <InfoBlock label="أسلوب الكتابة"    value={voice.writing_style    || voice.style}/>
              <InfoBlock label="سمات الشخصية"     value={voice.personality      || voice.sentence_structure}/>
              <InfoBlock label="مقاربة الكونتنت"  value={voice.content_approach || voice.cta_style}/>
            </div>

            {voice.do_use && voice.do_use.length > 0 && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <InfoBlock label="استخدم دائماً"  tags={voice.do_use}/>
                <InfoBlock label="تجنّب دائماً"   tags={voice.avoid}/>
              </div>
            )}

            {!voice.primary_tone && !voice.tone && (
              <div style={{ padding: '48px 24px', textAlign: 'center', color: 'rgba(255,255,255,0.30)', fontSize: '13px', lineHeight: 1.7 }}>
                ارفع كونتنت براندك وشغّل التحليل لاستخراج ملف صوتك.
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'الجمهور' && (
          <div className="b-in">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <InfoBlock label="الجمهور الأساسي"         value={aud.primary            || aud.demographics   || ''}/>
              <InfoBlock label="علم النفس"               value={aud.psychographics     || aud.psychology      || ''}/>
              <InfoBlock label="نقاط الألم"              tags={Array.isArray(aud.pain_points) ? aud.pain_points : [aud.pain_points].filter(Boolean)}/>
              <InfoBlock label="الأهداف والرغبات"         tags={Array.isArray(aud.goals)       ? aud.goals       : [aud.goals].filter(Boolean)}/>
              <InfoBlock label="الكونتنت اللي يحبونه"     value={aud.content_preferences}/>
              <InfoBlock label="المنصات اللي يستخدمونها"  tags={Array.isArray(aud.platforms)   ? aud.platforms   : []}/>
            </div>
        ...
        {tab === 'الهوية البصرية' && (
          <div className="b-in">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <InfoBlock label="الأسلوب البصري"       value={visual.aesthetic        || visual.style             || ''}/>
              <InfoBlock label="الخطوط"               value={visual.typography       || visual.fonts?.join(', ') || ''}/>
              <InfoBlock label="توجيه الصور"          value={visual.image_guidelines || visual.photography_style || ''}/>
              <InfoBlock label="كلمات الهوية البصرية" tags={visual.keywords || (visual.video_style ? [visual.video_style] : [])}/>
            </div>
          ...
          <div className="b-in">
            ...
            <div
              ...
              <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.30)', lineHeight: 1.65 }}>
                شعارات، منشورات نموذجية، صور منتجات، وثائق براند.<br/>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
                  ...
                  {assets.length >= 2 && !profile && (
                    <button onClick={analyze} disabled={analyzing}
                      ...
                      {analyzing
                        ? 'Nexa تحلّل...'
                        : <><span style={{ display: 'flex' }}>{Ic.bolt}</span>حلّلي الآن</>}
                    </button>
                  )}
                </div>
                      ...
                      <div key={a.id}
                        ...
                        <div style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.28)', marginTop: '2px' }}>{a.type?.replace('_', ' ') || 'asset'}</div>
                          </div>
                ...
                {profile && (
                  <div style={{ marginTop: '20px', padding: '16px 20px', background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.45)', lineHeight: 1.5 }}>
                      أضفت ملفات جديدة؟ أعد التحليل وخلّ Nexa تتحدّث.
                    </div>
                    <button onClick={analyze} disabled={analyzing}
                      style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 16px', fontSize: '12px', fontWeight: 600, background: '#FFFFFF', color: '#0C0C0C', border: 'none', borderRadius: '8px', cursor: 'pointer', flexShrink: 0, marginRight: '16px', transition: 'background 0.15s' }}
                      onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.88)'}
                      onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = '#FFFFFF'}>
                      <span style={{ display: 'flex' }}>{Ic.bolt}</span>
                      {analyzing ? 'Nexa تحلّل...' : 'حلّل من جديد'}
                    </button>
                  </div>
                )}
              </>
            ...
            {assets.length === 0 && (
              <div style={{ textAlign: 'center', padding: '40px', color: 'rgba(255,255,255,0.25)', fontSize: '13px', lineHeight: 1.7 }}>
                ما في ملفات بعد.<br/>
                <span style={{ color: 'rgba(255,255,255,0.18)', fontSize: '12px' }}>كلما أعطيت Nexa أكثر، كلما عرفتك أعمق.</span>
              </div>
            )}
          </div>
        )}
