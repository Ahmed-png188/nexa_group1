...
import { strategy as strategyCopy, t } from '@/content/copy'
...
export default function StrategyPage() {
  ...
  const c                  = t(strategyCopy, lang)

  const TABS = [
    { id:'overview'    as Tab, label: isArabic ? 'نظرة عامة'    : 'Overview'     },
    { id:'pillars'     as Tab, label: isArabic ? 'الركائز'       : 'Pillars'      },
    { id:'angles'      as Tab, label: isArabic ? 'الزوايا'       : 'Angles'       },
    { id:'plan'        as Tab, label: isArabic ? 'خطة 30 يوم'    : '30-Day Plan'  },
    { id:'timing'      as Tab, label: isArabic ? 'التوقيت'       : 'Timing'       },
    { id:'competitors' as Tab, label: isArabic ? 'المنافسون'     : 'Competitors'  },
  ]

  const F = isArabic ? "'Tajawal', sans-serif" : "'Geist', -apple-system, sans-serif"
  ...
  function EmptyState({ icon, title, desc, children }: any) {
    return (
  ...
  async function generateStrategy() {
    ...
    try {
      const r = await fetch('/api/generate-strategy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({workspace_id:ws.id, lang})})
      if (!r.ok) throw new Error('Strategy generation failed')
  ...
  async function generateTiming() {
    ...
    try { const r=await fetch('/api/strategy-timing',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({workspace_id:ws.id})}); if(r.ok){const d=await r.json(); if(d.success)setTimingData(d.timing)} } catch {}
    setGenTiming(false)
  }

  async function generateComp() {
    if (!ws||genComp) return; setGenComp(true)
    try { const r=await fetch('/api/competitor-analysis',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({workspace_id:ws.id,competitors:compInput,lang})}); if(r.ok){const d=await r.json(); if(d.success)setCompData(d.analysis)} } catch {}
    setGenComp(false)
  }
        ...
        <div style={{ maxWidth:'600px', margin:'0 auto', padding:'0 40px', textAlign:'center', position:'relative', zIndex:1 }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:'6px', padding:'5px 14px', background:'rgba(0,0,0,0.12)', border:'1px solid rgba(0,0,0,0.18)', borderRadius:'100px', marginBottom:'24px' }}>
            ...
            <span style={{ fontSize:'11px', color:'rgba(0,0,0,0.65)', fontWeight:600 }}>
              {isArabic ? 'الاستراتيجية · لم تُبنَ بعد' : 'Strategy · Not generated yet'}
            </span>
          </div>
          <h1 style={{ fontSize:'48px', fontWeight:700, letterSpacing:'-0.04em', color:'#0A0A0A', lineHeight:1.05, marginBottom:'16px', fontFamily:F }}>
            {isArabic ? <>استراتيجيتك<br/>تنتظرك.</> : <>Your strategy<br/>is waiting.</>}
          </h1>
          <p style={{ fontSize:'16px', color:'rgba(0,0,0,0.55)', lineHeight:1.7, marginBottom:'36px', fontFamily:F }}>
            {isArabic
              ...
              : 'Nexa analyzes your brand voice, audience psychology, and content patterns — then builds your complete content blueprint: pillars, angles, a 30-day plan, and competitive intelligence.'}
          </p>
          {!hasBrand && (
            <div style={{ marginBottom:'20px', padding:'10px 18px', background:'rgba(0,0,0,0.10)', border:'1px solid rgba(0,0,0,0.16)', borderRadius:'8px', display:'inline-flex', alignItems:'center', gap:'8px' }}>
              ...
              <span style={{ fontSize:'12px', color:'rgba(0,0,0,0.60)', fontFamily:F }}>
                {isArabic ? 'تشتغل أحسن لما تدرّب Brand Brain أول.' : 'Works best after Brand Brain is trained.'}
              </span>
              <a href="/dashboard/brand" style={{ fontSize:'12px', fontWeight:600, color:'rgba(0,0,80,0.75)', textDecoration:'none', flexShrink:0, fontFamily:F }}>
                {isArabic ? 'درّب Brand Brain ←' : 'Train Brand Brain →'}
              </a>
            </div>
          )}
      ...
      <div style={{ padding:'40px 36px' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'12px' }}>
          {(isArabic ? [
            ...
            { icon:Ic.star,   title:'Content Pillars',        desc:'The core themes that define what you talk about and why people follow you.' },
            { icon:Ic.bolt,   title:'Winning Angles',         desc:'The specific psychological hooks that make your content perform better than average.' },
            { icon:Ic.cal,    title:'30-Day Plan',            desc:'A complete day-by-day content calendar with angles, goals, and platform guidance.' },
            { icon:Ic.clock,  title:'Optimal Timing',         desc:'The exact windows when your specific audience is most receptive.' },
            { icon:Ic.target, title:'Competitor Intelligence', desc:'Where your competitors are weak and the white space you can own.' },
            { icon:Ic.chart,  title:'Audience Map',           desc:"Deep psychographic data about who you're talking to and what moves them." },
          ]).map((item,i) => (
  ...
  if (generating) return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'calc(100vh - var(--topbar-h))', gap:'16px', textAlign:'center', padding:'40px', background:'#0C0C0C', fontFamily:F }}>
      ...
      <div style={{ fontSize:'20px', fontWeight:600, letterSpacing:'-0.03em', color:'rgba(255,255,255,0.88)', fontFamily:F }}>
        {isArabic ? 'Nexa تبني استراتيجيتك...' : 'Building your strategy…'}
      </div>
      <div style={{ fontSize:'13px', color:'rgba(255,255,255,0.35)', lineHeight:1.7, maxWidth:'360px', fontFamily:F }}>
        {isArabic
          ? 'Nexa تحلّل براندك وجمهورك وأنماط كونتنتك.\nهذا يأخذ نحو 30 ثانية.'
          : 'Analyzing your brand, audience, and content patterns.\nThis takes about 30 seconds.'}
      </div>
      <div style={{ display:'flex', gap:'5px', marginTop:'8px' }}>
        {[0,1,2].map(i => <div key={i} style={{ width:6, height:6, borderRadius:'50%', background:'#00AAFF', animation:`pulse-dot 1.4s ease-in-out ${i*0.2}s infinite` }}/>)}
      </div>
    </div>
  )
    ...
    <div style={{ height:'calc(100vh - var(--topbar-h))', overflowY:'auto', background:'#0C0C0C', fontFamily:F }}>
      ...
      <div style={{ opacity:mounted?1:0, transition:'opacity 0.4s ease' }}>
        <div style={{ backgroundImage:'url(/cyan-header.png)', backgroundSize:'cover', backgroundPosition:'center top', padding:'40px 0 28px' }}>
          <div style={{ padding:'0 36px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:'20px' }}>
            <div>
              <h1 style={{ fontSize:'36px', fontWeight:700, letterSpacing:'-0.04em', color:'#0A0A0A', lineHeight:1, marginBottom:'8px', fontFamily:F }}>{c.heading}</h1>
              <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                ...
                <span style={{ fontSize:'13px', color:'rgba(0,0,0,0.60)', fontWeight:500, fontFamily:F }}>
                  {isArabic
                    ...
                    : `${pillars.length} pillars · ${angles.length} angles · ${planDays.length} days planned`}
                </span>
              </div>
            </div>
            <button onClick={generateStrategy} disabled={generating}
              ...
              {isArabic ? 'أعيدي البناء' : 'Regenerate'}
            </button>
          </div>
        </div>
      ...
      <div style={{ padding:'28px 36px 0' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px', marginBottom:'28px' }}>
          <StatCard icon={Ic.users} label={isArabic ? 'رؤى الجمهور'    : 'Audience insights'} value={Object.keys(aud).length||'—'}/>
          <StatCard icon={Ic.star}  label={isArabic ? 'الركائز'         : 'Content pillars'}   value={pillars.length||'—'}/>
          <StatCard icon={Ic.bolt}  label={isArabic ? 'الزوايا الرابحة' : 'Winning angles'}    value={angles.length||'—'}/>
          <StatCard icon={Ic.cal}   label={isArabic ? 'أيام مخططة'     : 'Days planned'}      value={planDays.length||'—'}/>
        </div>
      </div>
          ...
          <div className="s-in">
            {insights.length > 0 && (
              <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'24px', marginBottom:'16px' }}>
                <Label>{isArabic ? 'إيش لقت Nexa' : 'What Nexa Found'}</Label>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'12px' }}>
              {Object.entries(aud).slice(0,6).map(([k,v]: any) => {
                const label = isArabic
                  ...
                  : k.replace(/_/g,' ')
                return <InfoCard key={k} label={label} value={typeof v==='string'?v:Array.isArray(v)?v.join(', '):JSON.stringify(v)}/>
              })}
            </div>
            {Object.keys(aud).length === 0 && (
              <div style={{ textAlign:'center', padding:'48px', color:'rgba(255,255,255,0.25)', fontSize:'13px', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:'10px', fontFamily:F }}>
                {isArabic ? 'ما في بيانات جمهور. أعيدي بناء الاستراتيجية.' : 'No audience data. Regenerate your strategy.'}
              </div>
            )}
          </div>
        )}
            ...
            {pillars.length > 0 ? (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'12px' }}>
                {pillars.map((p: any, i: number) => {
                  const name   = typeof p==='string'?p:(p.name||p.pillar||`${isArabic?'ركيزة':'Pillar'} ${i+1}`)
                  ...
                  return (
                    <div key={i} style={{ padding:'22px', background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', transition:'border-color 0.15s' }}
                      ...
                      {example && <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.30)', fontStyle:'italic', marginBottom:topics.length?'10px':'0', fontFamily:F }}>e.g. "{example}"</div>}
                      {topics.length > 0 && (
              ...
              <div style={{ textAlign:'center', padding:'48px', color:'rgba(255,255,255,0.25)', fontSize:'13px', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:'10px', fontFamily:F }}>
                {isArabic ? 'ما في ركائز بعد. أعيدي بناء الاستراتيجية.' : 'No pillars yet. Regenerate your strategy.'}
              </div>
            )}
          </div>
        )}
            ...
            {angles.length > 0 ? (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'12px' }}>
                {angles.map((a: any, i: number) => {
                  const title = typeof a==='string'?a:(a.angle||a.title||`${isArabic?'زاوية':'Angle'} ${i+1}`)
                  ...
                  return (
                    <div key={i} style={{ padding:'22px', background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', transition:'border-color 0.15s' }}
                      ...
                      {hook && (
                        <div style={{ padding:'12px 14px', background:'rgba(0,170,255,0.04)', border:'1px solid rgba(0,170,255,0.12)', borderLeft: isArabic?'none':'3px solid rgba(0,170,255,0.40)', borderRight: isArabic?'3px solid rgba(0,170,255,0.40)':'none', borderRadius: isArabic?'8px 0 0 8px':'0 8px 8px 8px', marginBottom:'10px' }}>
                          <div style={{ fontSize:'9px', fontWeight:600, color:'rgba(0,170,255,0.80)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:'5px', fontFamily:F }}>{isArabic ? 'الهوك' : 'Hook'}</div>
              ...
              <div style={{ textAlign:'center', padding:'48px', color:'rgba(255,255,255,0.25)', fontSize:'13px', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:'10px', fontFamily:F }}>
                {isArabic ? 'ما في زوايا بعد. أعيدي بناء الاستراتيجية.' : 'No angles yet. Regenerate your strategy.'}
              </div>
            )}
          </div>
        )}
            ...
            {planDays.length > 0 ? (
              <>
                <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:'6px', marginBottom:'16px' }}>
                  {(isArabic ? DAYS_AR : DAYS_EN).map(d => (
                  ...
                  {planDays.map((day: any, i: number) => {
                    ...
                    return (
                      <div key={i}
                        ...
                        {isOpen && <div style={{ position:'absolute', top:0, left:0, right:0, height:'2px', background:'#00AAFF' }}/>}
                        ...
                        {hovDay === num && !isOpen && (
                          <button
                            ...
                            style={{ position:'absolute', bottom:'5px', right:'5px', padding:'2px 7px', borderRadius:'5px', fontSize:'9px', fontWeight:700, background:'#FFFFFF', color:'#0C0C0C', border:'none', cursor:'pointer', fontFamily:F }}>
                            {isArabic ? 'أنشئ' : 'Create'}
                          </button>
                        )}
                      </div>
                    )
                    ...
                    <div style={{ padding:'24px 28px', background:'#141414', border:'1px solid rgba(0,170,255,0.20)', borderTop:'3px solid #00AAFF', borderRadius:'10px', animation:'sUp 0.2s ease both' }}>
                      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'20px' }}>
                        <div>
                          <div style={{ fontSize:'11px', color:'rgba(255,255,255,0.30)', marginBottom:'4px', fontFamily:F }}>
                            {day.platform ? `${day.platform} · ` : ''}{isArabic ? `الأسبوع ${Math.ceil(expanded/7)}` : `Week ${Math.ceil(expanded/7)}`}
                          </div>
                          <div style={{ fontSize:'32px', fontWeight:700, color:'#FFFFFF', letterSpacing:'-0.05em', lineHeight:1, fontFamily:MONO, marginBottom:'4px' }}>
                            {isArabic ? `يوم ${String(expanded).padStart(2,'0')}` : `Day ${String(expanded).padStart(2,'0')}`}
                          </div>
                      ...
                      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', marginBottom:'20px' }}>
                        {day.angle && (
                          <div style={{ padding:'14px 16px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'8px', gridColumn: day.angle.length > 60 ? 'span 2' : 'span 1' }}>
                            <Label>{isArabic ? 'الزاوية' : 'Angle'}</Label>
                        ...
                        {(day.content||day.description) && (
                          <div style={{ padding:'14px 16px', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'8px', gridColumn:'span 2' }}>
                            <Label>{isArabic ? 'إيش تنشئ' : 'What to create'}</Label>
                      ...
                      <div style={{ display:'flex', gap:'8px' }}>
                        <button onClick={() => router.push(`/dashboard/studio?strategy_day=${expanded}&angle=${encodeURIComponent(day.angle||day.theme||'')}`)}
                          ...
                          {isArabic ? 'أنشئ في الاستوديو' : 'Create in Studio'} <span style={{ display:'flex' }}>{Ic.arrow}</span>
                        </button>
                        <button onClick={() => router.push(`/dashboard/studio?strategy_day=${expanded}&angle=${encodeURIComponent(day.angle||day.theme||'')}&format=reel`)}
                          ...
                          {isArabic ? 'اعملي ريلز' : 'Make a Reel'}
                        </button>
                      </div>
                    </div>
                  )
              ...
              <div style={{ textAlign:'center', padding:'48px', color:'rgba(255,255,255,0.25)', fontSize:'13px', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:'10px', fontFamily:F }}>
                {isArabic ? 'ما في خطة 30 يوم بعد. أعيدي بناء الاستراتيجية.' : 'No 30-day plan yet. Regenerate your strategy.'}
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'timing' && (
          <div className="s-in">
            {!timingData ? (
              <EmptyState
                icon={Ic.clock}
                title={isArabic ? 'محرّك التوقيت' : 'Timing Engine'}
                desc={isArabic
                  ? 'Nexa تحلّل جدول جمهورك اليومي وتجد اللحظات التي يكونون فيها أكثر تقبّلاً لكونتنتك.'
                  : "Nexa reverse-engineers your audience's daily schedule and finds the exact moments they're most receptive to your content."}>
                <button onClick={generateTiming} disabled={genTiming}
                  ...
                  {genTiming
                    ? <><div className="nexa-spinner" style={{ width:13, height:13 }}/>{isArabic ? 'Nexa تحلّل...' : 'Building…'}</>
                    : <><span style={{ display:'flex' }}>{Ic.clock}</span>{isArabic ? 'خرّطي أوقات نشري' : 'Map my posting times'}</>}
                </button>
              </EmptyState>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
                {timingData.analysis && (
                  <div style={{ padding:'20px 24px', background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px' }}>
                    <Label>{isArabic ? 'لماذا هذه الأوقات تشتغل' : 'Why these times work'}</Label>
                ...
                <button onClick={generateTiming} style={{ display:'flex', alignItems:'center', gap:'6px', padding:'7px 14px', fontSize:'12px', fontWeight:500, background:'transparent', border:'1px solid rgba(255,255,255,0.10)', color:'rgba(255,255,255,0.40)', borderRadius:'8px', cursor:'pointer', fontFamily:F, transition:'all 0.15s', alignSelf:'flex-start' as const }}
                  ...
                  <span style={{ display:'flex' }}>{Ic.refresh}</span>{isArabic ? 'أعيدي التحليل' : 'Regenerate'}
                </button>
              </div>
            )}
          </div>
            ...
            {!hasCompetitor ? (
              <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'60vh' }}>
                <div style={{ maxWidth:440, textAlign:'center', padding:'0 24px' }}>
                  ...
                  <div style={{ fontSize:19, fontWeight:700, letterSpacing:'-0.03em', color:'var(--text-1)', marginBottom:8, fontFamily:F }}>
                    {isArabic ? 'تحليل المنافسين — Grow+' : 'Competitor Analysis — Grow+'}
                  </div>
                  <div style={{ fontSize:13, color:'var(--text-3)', lineHeight:1.75, marginBottom:24, fontFamily:F }}>
                    {isArabic
                      ? 'Nexa ترسم المشهد التنافسي — من يملك أي تموضع، وأين الفجوات، وكيف تتميّز. متاح من خطة Grow ($89/شهر).'
                      : 'Nexa maps your competitive landscape — who owns what positioning, where the gaps are, and how to differentiate. Available on Grow ($89/mo) and above.'}
                  </div>
                  <a href="/dashboard/settings?tab=billing" style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'11px 24px', background:'var(--cyan)', color:'#000', borderRadius:10, fontWeight:700, fontSize:13, textDecoration:'none', letterSpacing:'-0.01em', fontFamily:F }}>
                    {isArabic ? 'طوّر إلى Grow ←' : 'Upgrade to Grow →'}
                  </a>
                  <div style={{ marginTop:12, fontSize:11, color:'var(--text-4)', fontFamily:F }}>
              ...
              <EmptyState
                icon={Ic.target}
                title={isArabic ? 'ذكاء المنافسين' : 'Competitor Intelligence'}
                desc={isArabic
                  ? 'Nexa ترسم المشهد التنافسي — من يملك أي تموضع، وأين الفجوات، وكيف تتميّز بطريقة لا يتحدث عنها أحد.'
                ...
                <div style={{ width:'100%', maxWidth:'480px', marginBottom:'16px', textAlign: isArabic ? 'right' : 'left' }}>
                  <div style={{ fontSize:'11px', fontWeight:500, color:'rgba(255,255,255,0.30)', marginBottom:'8px', fontFamily:F }}>
                    {isArabic ? 'مين منافسيك؟' : 'Who are your competitors?'} <span style={{ color:'rgba(255,255,255,0.18)' }}>({isArabic ? 'اختياري' : 'optional'})</span>
                  </div>
                  <textarea value={compInput} onChange={e => setCompInput(e.target.value)} rows={3}
                    placeholder={isArabic ? 'مثلاً: Buffer، Hootsuite، Later — أو صف مجالك' : 'e.g. Buffer, Hootsuite, Later — or describe your space'}
                    style={{ width:'100%', padding:'12px 14px', fontSize:'13px', fontFamily:F, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', color:'rgba(255,255,255,0.80)', outline:'none', resize:'vertical', lineHeight:1.65, transition:'border-color 0.15s', boxSizing:'border-box' as const, direction: isArabic ? 'rtl' : 'ltr' }}
                ...
                <button onClick={generateComp} disabled={genComp}
                  ...
                  {genComp
                    ...
                    : <><span style={{ display:'flex' }}>{Ic.target}</span>{isArabic ? 'شغّلي تحليل المنافسين' : 'Run competitor analysis'}</>}
                </button>
              </EmptyState>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
                {compData.market_position && (
                  <div style={{ padding:'20px 24px', background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px' }}>
                    <Label>{isArabic ? 'مكانتك في السوق' : 'Your Market Position'}</Label>
                ...
                {compData.differentiation_strategy?.positioning_statement && (
                  <div style={{ padding:'22px 26px', background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderLeft: isArabic?'none':'3px solid #00AAFF', borderRight: isArabic?'3px solid #00AAFF':'none', borderRadius: isArabic?'10px 0 0 10px':'0 10px 10px 10px' }}>
                    <Label>{isArabic ? 'تموضعك' : 'Your Positioning Statement'}</Label>
                ...
                {compData.white_space?.length > 0 && (
                  <div>
                    <Label>{isArabic ? 'الفرص غير المستغلة' : 'White Space Opportunities'}</Label>
                ...
                {compData.winning_angles?.length > 0 && (
                  <div>
                    <Label>{isArabic ? 'الزوايا التي يفوّتها المنافسون' : 'Winning Angles Competitors Miss'}</Label>
                ...
                {compData.competitors?.length > 0 && (
                  <div>
                    <Label>{isArabic ? 'تحليل المنافسين' : 'Competitor Breakdown'}</Label>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'12px', marginTop:'10px' }}>
                        ...
                        <div key={i} style={{ padding:'18px', background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px' }}>
                          ...
                          {comp.strengths?.length > 0 && (
                            <div style={{ marginBottom:'12px' }}>
                              <div style={{ fontSize:'9px', fontWeight:700, color:'#22C55E', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:'6px', fontFamily:F }}>{isArabic ? 'نقاط القوة' : 'Strengths'}</div>
                          ...
                          {comp.weaknesses?.length > 0 && (
                            <div style={{ marginBottom:'12px' }}>
                              <div style={{ fontSize:'9px', fontWeight:700, color:'#EF4444', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:'6px', fontFamily:F }}>{isArabic ? 'نقاط الضعف' : 'Weaknesses'}</div>
                          ...
                          {comp.differentiation_opportunity && (
                            <div style={{ padding:'9px 12px', background:'rgba(0,170,255,0.06)', border:'1px solid rgba(0,170,255,0.15)', borderRadius:'7px', fontSize:'11px', color:'#00AAFF', lineHeight:1.55, fontFamily:F }}>
                              {isArabic ? 'ميزتك: ' : 'Your edge: '}{comp.differentiation_opportunity}
                            </div>
                          )}
                        </div>
                      ))}
                ...
                <button onClick={generateComp} style={{ display:'flex', alignItems:'center', gap:'6px', padding:'7px 14px', fontSize:'12px', fontWeight:500, background:'transparent', border:'1px solid rgba(255,255,255,0.10)', color:'rgba(255,255,255,0.40)', borderRadius:'8px', cursor:'pointer', fontFamily:F, transition:'all 0.15s', alignSelf:'flex-start' as const }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor='rgba(255,255,255,0.22)'; (e.currentTarget as HTMLElement).style.color='rgba(255,255,255,0.70)' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor='rgba(255,255,255,0.10)'; (e.currentTarget as HTMLElement).style.color='rgba(255,255,255,0.40)' }}>
                  <span style={{ display:'flex' }}>{Ic.refresh}</span>{isArabic ? 'أعيدي التحليل' : 'Re-analyze'}
                </button>
              </div>
            )}
          </div>
