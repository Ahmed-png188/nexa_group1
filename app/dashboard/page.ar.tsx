...
import { agents as agentsCopy } from '@/content/copy'
...
const AGENTS = agentsCopy.ar

// Translate activity titles from English DB to Arabic display
function translateActivity(title: string): string {
...
function ReadyPostBodyAr({ post, hook }: { post: string; hook: string }) {
  ...
  return (
    <div>
      <div style={{
        ...
        fontFamily: F, letterSpacing:0, direction:'rtl',
      }}>
      ...
      {body.length > 120 && (
        <button
          ...
          style={{
            ...
            border:'none', padding:'4px 0', cursor:'pointer', fontFamily: F,
          }}
        >
...
export default function HomePageAr() {
  ...
  async function load() {
    ...
    try {
      const { data: runs } = await supabase.from('agent_runs').select('agent_id, agent_type, ran_at').eq('workspace_id', ws.id).order('ran_at',{ascending:false}).limit(20)
      ...
      runs?.forEach((run: any) => {
        ...
        if (type && !lastRunMap[type]) {
          ...
          lastRunMap[type] = isToday
            ? `اليوم ${d.toLocaleTimeString('ar',{hour:'2-digit',minute:'2-digit'})}`
  ...
  async function loadBrief(wsId: string) {
    ...
    const cacheKey = `nexa_brief_v3_${wsId}_${today}_ar`
    ...
    try {
      const r = await fetch('/api/morning-brief',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({workspace_id:wsId, lang:'ar'})})
  ...
  async function runAgent(agentType: string, agentName: string) {
    ...
    try {
      ...
      setAgentResults(prev => ({...prev,[agentName]: data.message || `${agentName} اكتمل`}))
      setResult({type:agentType,data})
      setTimeout(()=>load(),1000)
    } catch {
      setAgentResults(prev => ({...prev,[agentName]: 'صار خطأ — جرّب مرة ثانية'}))
  ...
  const statusLabels: Record<string,string> = {
    on_fire:       '🔥 براندك يشتعل',
    just_starting: 'بداية قوية',
    building:      'يتطور',
  }
  const statusCfg = brief?.status
    ? { label: statusLabels[brief.status] ?? brief.status, color: brief.status === 'on_fire' ? '#F59E0B' : brief.status === 'just_starting' ? '#22C55E' : '#00AAFF' }
    : null

  if (loading) return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'calc(100vh - 54px)',flexDirection:'column',gap:14,background:'#0C0C0C',fontFamily:F}}>
      <div className="nexa-spinner" style={{width:24,height:24}}/>
      <div style={{fontSize:12,color:'rgba(255,255,255,0.35)',fontFamily:F}}>يحمّل...</div>
    </div>
  )

  return (
    <div dir="rtl" style={{height:'calc(100vh - 54px)',overflowY:'auto',overflowX:'hidden',background:'#0C0C0C',fontFamily:F}}>
      ...
      <div style={{position:'relative',backgroundImage:'url(/cyan-header.png)',backgroundSize:'cover',backgroundPosition:'center top'}}>
        ...
        <div style={{position:'relative',zIndex:1,padding:'44px 40px 52px'}}>
          <div style={{marginBottom:'32px'}}>
            ...
            <div style={{fontSize:'11px',fontWeight:600,color:'rgba(0,0,0,0.55)',marginBottom:'12px',fontFamily:F}}>
              {greeting} · {format(new Date(), 'EEEE، d MMMM', { locale: ar })}
            </div>

            <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',gap:'20px',flexWrap:'wrap'}}>
              <div>
                <h1 style={{fontSize:'42px',fontWeight:700,color:'#0A0A0A',lineHeight:1,marginBottom:'8px',fontFamily:F}}>
                  {firstName}
                  <span style={{fontSize:'18px',fontWeight:500,color:'rgba(0,0,0,0.45)',marginRight:'14px',fontFamily:F}}>· مركز القيادة</span>
                </h1>
                ...
                {briefLoading ? (
                  <div style={{display:'flex',alignItems:'center',gap:'10px',height:'22px'}}>
                    ...
                    <span style={{fontSize:'13px',color:'rgba(0,0,0,0.45)',fontFamily:F}}>يقرأ الإشارات...</span>
                  </div>
                ) : brief?.headline ? (
                  <p style={{fontSize:'15px',color:'rgba(0,0,0,0.65)',lineHeight:1.55,maxWidth:'580px',margin:0,fontWeight:500,fontFamily:F}}>
                    {brief.headline}
                  </p>
                ) : (
                  <p style={{fontSize:'14px',color:'rgba(0,0,0,0.50)',margin:0,fontFamily:F}}>
                    {brandData ? 'براندك جاهز. الحين اكتب.' : (
                      <span>
                        ما درّبت Brand Brain بعد.{' '}
                        <a href="/dashboard/brand" style={{color:'#003399',textDecoration:'none',fontWeight:600}}>ارفع محتواك لتبدأ</a>
                      </span>
                    )}
                  </p>
                )}
              ...
              <div style={{display:'flex',alignItems:'center',gap:'8px',flexShrink:0}}>
                {statusCfg && (
                  <div style={{display:'flex',alignItems:'center',gap:'6px',padding:'7px 14px',background:'rgba(0,0,0,0.15)',border:'1px solid rgba(0,0,0,0.20)',borderRadius:'10px',fontSize:'12px',fontWeight:600,color:'rgba(0,0,0,0.70)',backdropFilter:'blur(8px)',fontFamily:F}}>
                    {brief?.status === 'on_fire' && <span style={{display:'flex'}}>{Ic.fire}</span>}
                    {statusCfg.label}
                  </div>
                )}
                {streak > 0 && (
                  <div style={{display:'flex',alignItems:'center',gap:'6px',padding:'7px 14px',background:'rgba(0,0,0,0.12)',border:'1px solid rgba(0,0,0,0.18)',borderRadius:'10px',fontSize:'12px',fontWeight:600,color:'rgba(0,0,0,0.65)',backdropFilter:'blur(8px)',fontFamily:F}}>
          ...
          {brief?.ready_post_hook && (
            <div style={{
              ...
              flexWrap:'wrap' as const, flexDirection:'row-reverse' as const,
            }}>
              <div style={{ flex:1, minWidth:200 }}>
                <div style={{
                  fontSize:'10px', fontWeight:700, letterSpacing:0,
                  color:'rgba(0,170,255,0.70)', marginBottom:5, fontFamily:F,
                }}>
                  جاهز للنشر
                </div>
                <div style={{
                  fontSize:'14px', fontWeight:600, color:'#FFFFFF',
                  lineHeight:1.35, fontFamily:F, letterSpacing:0,
                }}>
                  {brief.ready_post_hook}
                </div>
              </div>
          ...
          <div className="anim-1" style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',background:'rgba(20,20,20,0.80)',border:'1px solid rgba(255,255,255,0.10)',borderRadius:'12px',backdropFilter:'blur(20px)',overflow:'hidden'}}>
            {[
              { label:'الرصيد',       value:balance.toLocaleString('ar'),  tag:workspace?.plan||'spark',                                                           tagColor:'#00AAFF',  href:'/dashboard/settings?tab=billing' },
              { label:'المحتوى',      value:String(totalContent),          tag:`${content.filter((i:any)=>i.status==='published').length} منشور`,                  tagColor:'#22C55E',  href:'/dashboard/studio'               },
              { label:'المجدوَل',     value:String(scheduled.length),      tag:'قادمًا',                                                                           tagColor:'#F59E0B',  href:'/dashboard/schedule'             },
              { label:'التواصل',      value:streak>0?`${streak}`:  '—',   tag:streak>0?'استمر!':'ابدأ الآن',                                                      tagColor:streak>0?'#F59E0B':'rgba(255,255,255,0.25)', href:'/dashboard/schedule' },
              { label:'نقاء الصوت',   value:voiceScore>0?`${Math.round(voiceScore)}%`:'—', tag:voiceScore>80?'على المسار':voiceScore>60?'جيد':'درّب Brain Brain', tagColor:voiceScore>80?'#22C55E':voiceScore>60?'#F59E0B':'rgba(255,255,255,0.25)', href:'/dashboard/brand' },
            ].map((s,i) => (
              <a key={i} className="stat-item" href={s.href} style={{textDecoration:'none',padding:'20px 24px',borderRight:i>0?'1px solid rgba(255,255,255,0.08)':'none',display:'block',transition:'background 0.15s',background:'transparent'}}>
                <div style={{display:'flex',alignItems:'center',gap:'7px',marginBottom:'10px',flexWrap:'wrap'}}>
                  <span style={{fontSize:'10px',fontWeight:600,color:'rgba(255,255,255,0.35)',fontFamily:F}}>{s.label}</span>
                  <span style={{fontSize:'10px',fontWeight:700,color:s.tagColor,padding:'1px 7px',borderRadius:'4px',background:`${s.tagColor}18`,fontFamily:F}}>{s.tag}</span>
                </div>
                <div style={{fontSize:'28px',fontWeight:700,color:'#FFFFFF',lineHeight:1,fontFamily:MONO}}>{s.value}</div>
              </a>
            ))}
          </div>
        </div>
      ...
      <div style={{padding:'0 40px 48px'}}>
        ...
        {brief && (
          <div className="anim-2" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'12px',marginBottom:'28px'}}>
            ...
            <div className="brief-card" style={{padding:'20px 24px',background:'#141414',border:'1px solid rgba(255,255,255,0.10)',borderRight:'3px solid #00AAFF',borderRadius:'10px 0 0 10px',transition:'border-color 0.15s'}}>
              <div style={{fontSize:'10px',fontWeight:600,color:'rgba(0,170,255,0.7)',marginBottom:'10px',fontFamily:F}}>أولويتك اليوم</div>
              <div style={{fontSize:'13px',fontWeight:500,color:'rgba(255,255,255,0.85)',lineHeight:1.65,fontFamily:F}}>{brief.todays_priority}</div>
            </div>

            {brief.todays_angle && (
              <a href={`/dashboard/studio${brief.todays_angle?`?angle=${encodeURIComponent(brief.todays_angle)}`:''}`}
                className="brief-card"
                style={{padding:'20px 24px',background:'#141414',border:'1px solid rgba(255,255,255,0.10)',borderRight:'3px solid rgba(255,255,255,0.20)',borderRadius:'10px 0 0 10px',textDecoration:'none',display:'block',transition:'border-color 0.15s'}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px'}}>
                  <div style={{fontSize:'10px',fontWeight:600,color:'rgba(255,255,255,0.35)',fontFamily:F}}>اكتب هذا اليوم</div>
                  <span style={{color:'rgba(255,255,255,0.25)',display:'flex'}}>{Ic.arrow}</span>
                </div>
                <div style={{fontSize:'13px',fontWeight:500,color:'rgba(255,255,255,0.85)',lineHeight:1.65,fontFamily:F}}>{brief.todays_angle}</div>
                {brief.todays_platform && <div style={{fontSize:'11px',color:'rgba(255,255,255,0.30)',marginTop:'8px',fontFamily:F}}>الأفضل على {brief.todays_platform}</div>}
              </a>
            )}

            <div className="brief-card" style={{padding:'20px 24px',background:'#141414',border:'1px solid rgba(255,255,255,0.10)',borderRight:'3px solid rgba(255,255,255,0.12)',borderRadius:'10px 0 0 10px',transition:'border-color 0.15s'}}>
              <div style={{fontSize:'10px',fontWeight:600,color:'rgba(255,255,255,0.35)',marginBottom:'10px',fontFamily:F}}>شيء واحد</div>
              <div style={{fontSize:'13px',fontWeight:500,color:'rgba(255,255,255,0.85)',lineHeight:1.65,fontFamily:F}}>{brief.one_thing}</div>
            </div>
          </div>
        )}
        ...
        {brief?.ready_post && (
          <div style={{
            ...
            {/* Header */}
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12, flexDirection:'row-reverse' as const }}>
              <div style={{ display:'flex', alignItems:'center', gap:7, flexDirection:'row-reverse' as const }}>
                <div style={{ width:6, height:6, borderRadius:'50%', background:'#00AAFF' }}/>
                <span style={{
                  fontSize:'10px', fontWeight:700, letterSpacing:0,
                  color:'rgba(0,170,255,0.80)', fontFamily:F,
                }}>
                  انشر اليوم
                </span>
              </div>
              {brief.todays_platform && (
                <span style={{
                  ...
                  color:'rgba(255,255,255,0.40)', fontFamily:F, letterSpacing:0,
                }}>
                  {brief.todays_platform}
                </span>
              )}
            ...
            <div style={{
              ...
              lineHeight:1.5, marginBottom:8, fontFamily:F, letterSpacing:0,
            }}>
              {brief.ready_post_hook}
            </div>

            {/* Post body - collapsible */}
            <ReadyPostBodyAr post={brief.ready_post} hook={brief.ready_post_hook || ''} />

            {/* Action buttons */}
            <div style={{ display:'flex', gap:8, marginTop:14, flexWrap:'wrap' as const, flexDirection:'row-reverse' as const }}>
              <button
                onClick={() => {
                  localStorage.setItem('nexa_ready_post', brief.ready_post)
                  localStorage.setItem('nexa_ready_platform', brief.todays_platform || 'general')
        ...
        {(brief?.warning || (voiceDrift?.drift_detected && voiceDrift.drift_severity !== 'none')) && (
          <div style={{display:'flex',alignItems:'flex-start',gap:'12px',padding:'16px 20px',background:'rgba(245,158,11,0.06)',border:'1px solid rgba(245,158,11,0.20)',borderRight:'3px solid #F59E0B',borderRadius:'10px 0 0 10px',marginBottom:'28px'}}>
            <span style={{display:'flex',color:'#F59E0B',flexShrink:0,marginTop:'1px'}}>{Ic.warning}</span>
            <div style={{flex:1}}>
              <div style={{fontSize:'13px',fontWeight:600,color:'#F59E0B',marginBottom:'4px',fontFamily:F}}>انجراف الصوت</div>
              <div style={{fontSize:'12px',color:'rgba(255,255,255,0.50)',lineHeight:1.6,fontFamily:F}}>{voiceDrift?.alert_message||brief?.warning}</div>
            </div>
            <a href="/dashboard/brand" style={{flexShrink:0,fontSize:'12px',fontWeight:600,color:'#F59E0B',textDecoration:'none',padding:'6px 14px',background:'rgba(245,158,11,0.10)',border:'1px solid rgba(245,158,11,0.25)',borderRadius:'8px',whiteSpace:'nowrap',fontFamily:F}}>أعد المعايرة</a>
          </div>
        )}
        ...
        <div className="anim-3" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'12px'}}>
          ...
          <div style={{background:'#141414',border:'1px solid rgba(255,255,255,0.10)',borderRadius:'10px',padding:'24px'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'20px'}}>
              <div>
                <div style={{fontSize:'15px',fontWeight:600,color:'#FFFFFF',marginBottom:'3px',fontFamily:F}}>وكلاء الذكاء</div>
                <div style={{fontSize:'12px',color:'rgba(255,255,255,0.35)',fontFamily:F}}>كل واحد متخصص. شغّله وشوف الفرق</div>
              </div>
              <a href="/dashboard/automate" style={{fontSize:'12px',fontWeight:500,color:'rgba(255,255,255,0.35)',textDecoration:'none',display:'flex',alignItems:'center',gap:'4px',transition:'color 0.15s',fontFamily:F}}
                ...
                return (
                  <div key={agent.id} className="agent-row" style={{display:'flex',alignItems:'center',gap:'12px',padding:'12px 14px',borderRadius:'8px',background:isRunning?'rgba(0,170,255,0.06)':'rgba(255,255,255,0.02)',border:`1px solid ${isRunning?'rgba(0,170,255,0.20)':'rgba(255,255,255,0.06)'}`,transition:'all 0.15s'}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:'13px',fontWeight:500,color:'rgba(255,255,255,0.88)',marginBottom:'2px',fontFamily:F}}>{agent.name}</div>
                      <div style={{fontSize:'11px',color:'rgba(255,255,255,0.30)',lineHeight:1.4,fontFamily:F}}>
                        {resultMsg
                          ? <span style={{color:'#22C55E'}}>{resultMsg}</span>
                          : lastRunStr ? `آخر تشغيل: ${lastRunStr}`
                          : agent.desc}
                      </div>
                    </div>
                    <button onClick={()=>runAgent(agent.id, agent.name)} disabled={!!agentRunning}
                      style={{display:'flex',alignItems:'center',gap:'5px',padding:'6px 12px',borderRadius:'7px',fontSize:'11px',fontWeight:600,background:isRunning?'#00AAFF':'transparent',border:`1px solid ${isRunning?'transparent':'rgba(255,255,255,0.10)'}`,color:isRunning?'#000':'rgba(255,255,255,0.40)',cursor:agentRunning?'not-allowed':'pointer',transition:'all 0.15s',fontFamily:F}}
                      ...
                      {isRunning
                        ? <><div className="nexa-spinner" style={{width:'10px',height:'10px'}}/>يشتغل...</>
                        : <><span style={{display:'flex'}}>{Ic.play}</span>شغّل</>}
                    </button>
                  </div>
                )
              })}
              ...
              <div style={{marginTop:'14px',padding:'14px 16px',background:'rgba(34,197,94,0.06)',border:'1px solid rgba(34,197,94,0.20)',borderRadius:'8px',animation:'fadeUp 0.3s ease both'}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'6px'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
                    ...
                    <span style={{fontSize:'12px',fontWeight:600,color:'#22C55E',fontFamily:F}}>
                      {AGENTS.find(a=>a.id===result.type)?.name} اكتمل
                    </span>
                  </div>
                ...
                <div style={{fontSize:'12px',color:'rgba(255,255,255,0.50)',lineHeight:1.6,fontFamily:F}}>
                  {result.data.posts?.length>0
                    ? <>{result.data.posts.length} منشور حُفظ. <a href="/dashboard/studio" style={{color:'#00AAFF',textDecoration:'none',fontWeight:600}}>افتح الاستوديو</a></>
                    : result.data.message || 'اكتمل.'}
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            ...
            <div style={{background:'#141414',border:'1px solid rgba(255,255,255,0.10)',borderRadius:'10px',padding:'24px',flex:1}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'20px'}}>
                <div>
                  <div style={{fontSize:'15px',fontWeight:600,color:'#FFFFFF',marginBottom:'3px',fontFamily:F}}>قائمة النشر</div>
                  <div style={{fontSize:'12px',color:'rgba(255,255,255,0.35)',fontFamily:F}}>
                    {scheduled.length > 0 ? `${scheduled.length} منشور مجدوَل` : 'القائمة فاضية'}
                  </div>
                </div>
                <a href="/dashboard/schedule" style={{fontSize:'12px',fontWeight:500,color:'rgba(255,255,255,0.35)',textDecoration:'none',display:'flex',alignItems:'center',gap:'4px',transition:'color 0.15s',fontFamily:F}}
                  onMouseEnter={e=>(e.currentTarget as HTMLElement).style.color='rgba(255,255,255,0.80)'}
                  onMouseLeave={e=>(e.currentTarget as HTMLElement).style.color='rgba(255,255,255,0.35)'}>
                  أدر {Ic.arrow}
                </a>
              ...
              {scheduled.length > 0 ? (
                <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                  {scheduled.map(item=>(
                    <div key={item.id} style={{display:'flex',alignItems:'center',gap:'10px',padding:'10px 12px',background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:'8px'}}>
                      ...
                      <span style={{fontSize:'12px',color:'rgba(255,255,255,0.65)',flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontFamily:F}}>{translateActivity(item.title||'') || item.body?.slice(0,45) || 'منشور'}</span>
                      <span style={{fontSize:'11px',color:'rgba(255,255,255,0.25)',flexShrink:0,fontFamily:F}}>
                        {item.scheduled_for ? format(new Date(item.scheduled_for), 'd MMM · ha', { locale: ar }) : '—'}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center',padding:'24px 0',gap:'10px'}}>
                  <div style={{fontSize:'13px',fontWeight:500,color:'rgba(255,255,255,0.35)',fontFamily:F}}>ما في منشورات مجدوَلة</div>
                  <div style={{fontSize:'12px',color:'rgba(255,255,255,0.20)',fontFamily:F}}>أنشئ محتوى وجدوله من الاستوديو</div>
                  <a href="/dashboard/studio" style={{marginTop:'4px',fontSize:'12px',fontWeight:600,color:'#0C0C0C',background:'#FFFFFF',padding:'7px 16px',borderRadius:'8px',textDecoration:'none',display:'inline-flex',alignItems:'center',gap:'6px',transition:'background 0.15s',fontFamily:F}}
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              {[
                { label:'اكتب محتوى',   sub:'الاستوديو',    href:'/dashboard/studio',   icon:Ic.copy  },
                { label:'الاستراتيجية', sub:'خطة 30 يوم',   href:'/dashboard/strategy', icon:Ic.star  },
                { label:'الجدول',       sub:'انشر في كل مكان', href:'/dashboard/schedule', icon:Ic.cal   },
                { label:'Brand Brain',  sub:'هوية براندك',   href:'/dashboard/brand',    icon:Ic.brain },
              ].map(a=>(
                <a key={a.label} href={a.href} className="qa-card" style={{display:'flex',flexDirection:'column',gap:'12px',padding:'16px',background:'#141414',border:'1px solid rgba(255,255,255,0.10)',borderRadius:'10px',textDecoration:'none',transition:'all 0.15s'}}>
                  ...
                  <div>
                    <div style={{fontSize:'12px',fontWeight:600,color:'rgba(255,255,255,0.88)',marginBottom:'2px',fontFamily:F}}>{a.label}</div>
                    <div style={{fontSize:'11px',color:'rgba(255,255,255,0.30)',fontFamily:F}}>{a.sub}</div>
                  </div>
                </a>
              ))}
            </div>
        ...
        <div className="anim-4" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
          ...
          <div style={{background:'#141414',border:'1px solid rgba(255,255,255,0.10)',borderRadius:'10px',padding:'24px'}}>
            <div style={{marginBottom:'20px'}}>
              <div style={{fontSize:'15px',fontWeight:600,color:'#FFFFFF',marginBottom:'3px',fontFamily:F}}>النشاط</div>
              <div style={{fontSize:'12px',color:'rgba(255,255,255,0.35)',fontFamily:F}}>كل شيء يصير في الوقت الفعلي</div>
            </div>
            {activity.length > 0 ? (
              <div style={{display:'flex',flexDirection:'column'}}>
                {activity.slice(0,7).map((item,i)=>(
                  <div key={item.id} className="activity-row" style={{display:'flex',alignItems:'flex-start',gap:'12px',padding:'10px 8px',borderBottom:i<6?'1px solid rgba(255,255,255,0.05)':'none',borderRadius:'6px',transition:'background 0.12s',marginRight:'-8px',marginLeft:'-8px'}}>
                    <div style={{width:'6px',height:'6px',borderRadius:'50%',flexShrink:0,marginTop:'5px',background:actColor(item.type)}}/>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:'12px',color:'rgba(255,255,255,0.65)',lineHeight:1.5,fontFamily:F}}>{translateActivity(item.title)}</div>
                      <div style={{fontSize:'11px',color:'rgba(255,255,255,0.25)',marginTop:'2px',fontFamily:F}}>
                        {item.created_at ? `منذ ${formatDistanceToNow(new Date(item.created_at), { locale: ar })}` : ''}
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center',padding:'32px 0',gap:'8px'}}>
                <div style={{fontSize:'13px',fontWeight:500,color:'rgba(255,255,255,0.25)',fontFamily:F}}>ما في نشاط بعد</div>
                <div style={{fontSize:'12px',color:'rgba(255,255,255,0.15)',fontFamily:F}}>ابدأ بإنشاء محتوى لترى ما يصير هنا</div>
              </div>
            )}
          </div>
          ...
          <div style={{background:'#141414',border:'1px solid rgba(255,255,255,0.10)',borderRadius:'10px',padding:'24px'}}>
            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'20px'}}>
              <div>
                <div style={{fontSize:'15px',fontWeight:600,color:'#FFFFFF',marginBottom:'3px',fontFamily:F}}>ذكاء البراند</div>
                <div style={{fontSize:'12px',color:'rgba(255,255,255,0.35)',fontFamily:F}}>{brandData ? 'Brand Brain نشط ويتعلم' : 'Brand Brain ما اتدرّب بعد'}</div>
              </div>
              <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
                {brandData && (
                  <div style={{display:'flex',alignItems:'center',gap:'5px'}}>
                    <div style={{width:'6px',height:'6px',borderRadius:'50%',background:'#22C55E'}}/>
                    <span style={{fontSize:'11px',color:'#22C55E',fontWeight:600,fontFamily:F}}>نشط</span>
                  </div>
                )}
                {brandData && (
                  <button onClick={checkVoiceDrift} style={{fontSize:'11px',fontWeight:500,color:'rgba(255,255,255,0.35)',background:'transparent',border:'1px solid rgba(255,255,255,0.10)',borderRadius:'7px',padding:'5px 10px',cursor:'pointer',transition:'all 0.15s',fontFamily:F}}
            ...
            {brandData ? (
              <div>
                <div style={{display:'flex',flexDirection:'column',gap:'14px',marginBottom:'20px'}}>
                  {[
                    { label:'تطابق الصوت',  score:brandData.voice_match_score||voiceScore||0 },
                    { label:'ملاءمة الجمهور', score:brandData.audience_fit_score||0         },
                    { label:'الأسلوب البصري', score:brandData.visual_style_score||0          },
                  ].map(m=>(
                    <div key={m.label}>
                      <div style={{display:'flex',justifyContent:'space-between',marginBottom:'7px'}}>
                        <span style={{fontSize:'12px',color:'rgba(255,255,255,0.40)',fontFamily:F}}>{m.label}</span>
                        <span style={{fontSize:'12px',fontWeight:600,color:'rgba(255,255,255,0.88)',fontFamily:MONO}}>{m.score>0?`${m.score}%`:'—'}</span>
                      </div>
                ...
                {learnings.length > 0 && (
                  <div style={{borderTop:'1px solid rgba(255,255,255,0.06)',paddingTop:'16px'}}>
                    <div style={{fontSize:'10px',fontWeight:600,color:'rgba(255,255,255,0.25)',marginBottom:'12px',fontFamily:F}}>إيش تعلّمته NEXA</div>
                    {learnings.slice(0,3).map(l=>(
                      <div key={l.id} style={{display:'flex',gap:'10px',marginBottom:'10px',alignItems:'flex-start'}}>
                        <span style={{fontSize:'10px',fontWeight:600,padding:'2px 7px',borderRadius:'4px',flexShrink:0,height:'fit-content',marginTop:'1px',background:l.source==='performance_analysis'?'rgba(34,197,94,0.08)':'rgba(245,158,11,0.08)',border:`1px solid ${l.source==='performance_analysis'?'rgba(34,197,94,0.20)':'rgba(245,158,11,0.20)'}`,color:l.source==='performance_analysis'?'#22C55E':'#F59E0B',fontFamily:F}}>
                        ...
                        <div style={{fontSize:'12px',color:'rgba(255,255,255,0.40)',lineHeight:1.55,fontFamily:F}}>
                ...
                {brief?.insight && (
                  <div style={{marginTop:'16px',padding:'14px 16px',background:'rgba(0,170,255,0.05)',border:'1px solid rgba(0,170,255,0.15)',borderRight:'3px solid rgba(0,170,255,0.50)',borderRadius:'8px 0 0 8px'}}>
                    <div style={{fontSize:'10px',fontWeight:600,color:'rgba(0,170,255,0.70)',marginBottom:'6px',fontFamily:F}}>إنسايت Nexa</div>
                    <div style={{fontSize:'12px',color:'rgba(255,255,255,0.55)',lineHeight:1.65,fontFamily:F}}>{brief.insight}</div>
                  </div>
                )}
              </div>
            ) : (
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center',padding:'28px 0',gap:'12px'}}>
                ...
                <div style={{fontSize:'14px',fontWeight:600,color:'rgba(255,255,255,0.45)',fontFamily:F}}>Brand Brain ما اتدرّب</div>
                <div style={{fontSize:'12px',color:'rgba(255,255,255,0.25)',lineHeight:1.65,maxWidth:'260px',fontFamily:F}}>ارفع محتواك وسجّل صوتك، Nexa تبني هويتك الرقمية</div>
                <a href="/dashboard/brand" style={{marginTop:'4px',fontSize:'12px',fontWeight:600,color:'#0C0C0C',background:'#FFFFFF',padding:'7px 16px',borderRadius:'8px',textDecoration:'none',display:'inline-flex',alignItems:'center',gap:'6px',transition:'background 0.15s',fontFamily:F}}
                  onMouseEnter={e=>(e.currentTarget as HTMLElement).style.background='rgba(255,255,255,0.88)'}
                  onMouseLeave={e=>(e.currentTarget as HTMLElement).style.background='#FFFFFF'}>
                  درّب Brain Brain {Ic.arrow}
                </a>
              </div>
            )}
          </div>
      ...
      {toastMsg && (
        <div style={{ position:'fixed', bottom:24, right:'50%', transform:'translateX(50%)', zIndex:9999, padding:'9px 18px', background:'#1A1A1A', border:'1px solid rgba(255,255,255,0.12)', borderRadius:8, fontSize:'12px', color:'rgba(255,255,255,0.80)', fontFamily:F, letterSpacing:0, pointerEvents:'none' }}>
          {toastMsg}
        </div>
      )}
    </div>
