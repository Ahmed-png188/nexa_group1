      ...
      <div style={{ opacity:mounted?1:0, transition:'opacity 0.4s ease' }}>
        <div style={{ backgroundImage:'url(/cyan-header.png)', backgroundSize:'cover', backgroundPosition:'center top', padding:'40px 0 28px' }}>
          <div style={{ padding:'0 36px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:'20px' }}>
            <div>
              <h1 style={{ fontSize:'36px', fontWeight:700, letterSpacing:'-0.04em', color:'#0A0A0A', lineHeight:1, marginBottom:'8px' }}>Insights</h1>
      ...
      <div style={{ padding:'28px 36px 60px' }}>
        ...
        {!insights && (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'50vh', textAlign:'center', gap:'16px', fontFamily:F }}>
            ...
            <div style={{ fontSize:'16px', fontWeight:600, color:'rgba(255,255,255,0.70)', letterSpacing:'-0.02em' }}>No data yet for this period</div>
        ...
        {insights && (
          <>
              ...
              <div className="i-in">
                ...
                {connPlats.length===0 && insights.posts_created===0 && (
                  <div style={{ marginBottom:'24px', padding:'20px 24px', background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', textAlign:'center' }}>
                    <div style={{ fontSize:'14px', fontWeight:600, color:'rgba(255,255,255,0.70)', marginBottom:'6px', fontFamily:F }}>No performance data yet</div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'12px', marginBottom:'16px' }}>
                  <StatCard icon={Ic.eye}   label="Impressions"   value={fmt(insights.impressions)}         rawData={spark('impressions')}       color="#00AAFF" trend={insights?.trends?.impressions}/>
                  <StatCard icon={Ic.users} label="Reach"         value={fmt(insights.reach)}               rawData={spark('reach')}             color="#00AAFF" trend={insights?.trends?.reach}/>
                  <StatCard icon={Ic.heart} label="Engagements"   value={fmt(insights.engagement)}          rawData={spark('engagement')}        color="#00AAFF" trend={insights?.trends?.engagement}/>
                  <StatCard icon={Ic.click} label="Link Clicks"   value={fmt(insights.clicks)}              rawData={spark('clicks')}            color="#22C55E" trend={insights?.trends?.clicks}/>
                  <StatCard icon={Ic.chart} label="Posts Created" value={fmt(insights.posts_created||0)}    rawData={spark('posts_created')}     color="#00AAFF" trend={insights?.trends?.posts_created}/>
                  <StatCard icon={Ic.users} label="New Followers" value={fmt(insights.followers_gained||0)} rawData={spark('followers_gained')}  color="#00AAFF" trend={3}/>
                </div>
                ...
                <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px 24px', marginBottom:'12px' }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px' }}>
                    <div>
                      <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', marginBottom:'4px', fontFamily:F }}>Impressions over time</div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'16px' }}>
                  {[
                    { label:'Engagement rate', pct:engRate, sub:`${engRate.toFixed(2)}% of reach`, desc:'Engagements ÷ reach' },
                    { label:'Click-through rate', pct:ctr, sub:`${ctr.toFixed(2)}% of impressions`, desc:'Clicks ÷ impressions' },
                  ].map(m => (
                ...
                {aiText ? (
                  <div style={{ background:'#141414', border:'1px solid rgba(0,170,255,0.18)', borderLeft:'3px solid #00AAFF', borderRadius:'0 10px 10px 0', padding:'20px 24px' }}>
                    <div style={{ fontSize:'10px', fontWeight:600, color:'rgba(0,170,255,0.70)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:'10px', fontFamily:F }}>AI Analysis</div>
                  ...
                  <button onClick={explain} disabled={explaining}
                    ...
                    {explaining ? <><div className="nexa-spinner" style={{ width:12, height:12 }}/>Reading your numbers…</> : <><span style={{ display:'flex' }}>{Ic.bolt}</span>What does this mean?</>}
                  </button>
                )}
              </div>
            )}
            ...
            {tab==='content' && (
              <div className="i-in">
                <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px', marginBottom:'16px' }}>
                  ...
                  <StatCard icon={Ic.eye}   label="Total Impressions" value={fmt(insights.impressions)}      rawData={spark('impressions')}   color="#00AAFF"/>
                  <StatCard icon={Ic.heart} label="Total Engagements" value={fmt(insights.engagement)}       rawData={spark('engagement')}    color="#00AAFF"/>
                  <StatCard icon={Ic.star}  label="Avg per Post"      value={insights.posts_created>0?fmt(Math.round(insights.impressions/(insights.posts_created||1))):'—'} color="#22C55E"/>
                </div>
                <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px 24px', marginBottom:'12px' }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px' }}>
                    <div>
                      <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(0,170,255,0.70)', marginBottom:'4px', fontFamily:F }}>Engagement over time</div>
            ...
            {tab==='email' && (
              <div className="i-in">
                <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px', marginBottom:'16px' }}>
                  <StatCard icon={Ic.mail}  label="Emails Sent" value={fmt(insights.email_sent||0)}   rawData={spark('email_sent')}   color="#22C55E"/>
                  <StatCard icon={Ic.eye}   label="Opens"       value={fmt(insights.email_opens||0)}  rawData={spark('email_opens')}  color="#00AAFF"/>
                  <StatCard icon={Ic.click} label="Clicks"      value={fmt(insights.email_clicks||0)} rawData={spark('email_clicks')} color="#00AAFF"/>
                  <StatCard icon={Ic.chart} label="Open Rate"   value={pct(insights.email_opens||0,insights.email_sent||0)} color="#00AAFF"/>
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'16px' }}>
                  {[
                ...
                {connPlats.length>0 ? (
                  <>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'12px' }}>
                      <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px 24px' }}>
                        <div style={{ fontSize:'10px', fontWeight:600, color:'rgba(0,170,255,0.70)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:'18px', fontFamily:F }}>Impressions by Platform</div>
                      ...
                      <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px', padding:'20px 24px' }}>
                        <div style={{ fontSize:'10px', fontWeight:600, color:'rgba(0,170,255,0.70)', letterSpacing:'0.08em', textTransform:'uppercase', marginBottom:'18px', fontFamily:F }}>Engagement by Platform</div>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'50vh', textAlign:'center', gap:'16px', fontFamily:F }}>
                    <div style={{ width:52, height:52, borderRadius:'14px', background:'rgba(0,170,255,0.08)', border:'1px solid rgba(0,170,255,0.18)', display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(0,170,255,0.60)' }}>{Ic.chart}</div>
                    <div style={{ fontSize:'16px', fontWeight:600, color:'rgba(255,255,255,0.65)', fontFamily:F }}>No platforms connected yet</div>
