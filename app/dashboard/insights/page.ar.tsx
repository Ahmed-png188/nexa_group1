...
type Tab = 'نظرة عامة' | 'المحتوى' | 'الإيميل' | 'المنصات'
...
export default function InsightsPageAr() {
  ...
  const TABS: Tab[] = ['نظرة عامة', 'المحتوى', 'الإيميل', 'المنصات']
  ...
  async function sync() {
    ...
    try {
      await fetch('/api/sync-platform-data', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspace_id: ws.id }) })
      load()
    } catch {}
    setSyncing(false)
  }
  ...
  async function explain() {
    ...
    try {
      const summary = `المنشورات: ${insights.posts_created || 0}، الإمبريشنز: ${insights.impressions || 0}، التفاعل: ${insights.engagement || 0}، الوصول: ${insights.reach || 0}، النقرات: ${insights.clicks || 0} في آخر ${period} يوم.`
      const r = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          workspace_id: ws.id,
          message: `حلّل أداء محتواي لآخر ${period} يوم. أعطني 3 إنسايتس محددة وقابلة للتنفيذ. البيانات: ${summary} ايش الشيء الناجح، وايش يحتاج تحسين، وإيش أركز عليه الأسبوع القادم؟ كن مباشراً.`,
          history: [],
        }),
      })
      const d = await r.json(); setAiText(d.reply || 'ما في إنسايتس متاحة بعد.')
    } catch {}
    setExplaining(false)
  }
  ...
  const platBreak = connPlats.map(cp => {
    ...
    return {
      ...
      account:     cp.platform_username || cp.account_name || '',
      impressions: pr.reduce((a, r) => a + (r.impressions || 0), 0),
      engagement:  pr.reduce((a, r) => a + (r.engagement  || 0), 0),
      followers:   pr.reduce((a, r) => a + (r.followers_gained || 0), 0),
      clicks:      pr.reduce((a, r) => a + (r.clicks       || 0), 0),
    }
  })
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <div style={{ display: 'flex', background: 'rgba(0,0,0,0.12)', border: '1px solid rgba(0,0,0,0.16)', borderRadius: '10px', padding: '3px', gap: '2px' }}>
                {[
                  ...
                  <button key={d.v} onClick={() => setPeriod(d.v)}
                    style={{ padding: '5px 13px', borderRadius: '7px', fontSize: '12px', fontWeight: period === d.v ? 600 : 400, background: period === d.v ? 'rgba(0,0,0,0.18)' : 'transparent', border: `1px solid ${period === d.v ? 'rgba(0,0,0,0.22)' : 'transparent'}`, color: period === d.v ? '#0A0A0A' : 'rgba(0,0,0,0.50)', cursor: 'pointer', fontFamily: F, transition: 'all 0.15s' }}>
                    {d.l}
                  </button>
                ))}
              </div>
      ...
      <div style={{ padding: '28px 36px 60px' }}>
        ...
        {showBanner && (
          <div style={{ padding: '12px 18px', background: 'rgba(0,170,255,0.05)', border: '1px solid rgba(0,170,255,0.18)', borderRadius: '10px', fontSize: '13px', color: 'rgba(255,255,255,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', marginBottom: '24px', fontFamily: F }}>
            <span>وصّل منصة نشر لمزامنة بياناتك ومتابعة أدائك</span>
            <Link href="/dashboard/integrations" style={{ fontSize: '12px', color: '#00AAFF', textDecoration: 'none', fontWeight: 600, whiteSpace: 'nowrap' as const }}>
              وصّل المنصات <span style={{ display: 'inline-flex' }}>{Ic.arrow}</span>
            </Link>
          </div>
        )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '50vh', textAlign: 'center', gap: '16px', fontFamily: F }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={sync} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '10px 20px', background: '#FFFFFF', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: 600, color: '#0C0C0C', cursor: 'pointer', fontFamily: F }}>
                <span style={{ display: 'flex' }}>{Ic.refresh}</span>زامن المنصات
              </button>
        ...
        {insights && (
          <>
              ...
              <div className="i-in">
                ...
                {connPlats.length === 0 && insights.posts_created === 0 && (
                  <div style={{ marginBottom: '24px', padding: '20px 24px', background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', textAlign: 'center' }}>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: 'rgba(255,255,255,0.70)', marginBottom: '6px', fontFamily: F }}>ما في بيانات أداء بعد</div>
                    <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', lineHeight: 1.7, maxWidth: '340px', margin: '0 auto 16px', fontFamily: F }}>
                      وصّل منصاتك الاجتماعية لتتابع الإمبريشنز والوصول والتفاعل بشكل لحظي.
                    </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px', marginBottom: '16px' }}>
                  <StatCard icon={Ic.eye}   label="الإمبريشنز"   value={fmt(insights.impressions)}         rawData={spark('impressions')}       color="#00AAFF" trend={insights?.trends?.impressions}/>
                  <StatCard icon={Ic.users} label="الوصول"         value={fmt(insights.reach)}               rawData={spark('reach')}             color="#00AAFF" trend={insights?.trends?.reach}/>
                  <StatCard icon={Ic.heart} label="التفاعل"        value={fmt(insights.engagement)}          rawData={spark('engagement')}        color="#00AAFF" trend={insights?.trends?.engagement}/>
                  <StatCard icon={Ic.click} label="النقرات"         value={fmt(insights.clicks)}              rawData={spark('clicks')}            color="#22C55E" trend={insights?.trends?.clicks}/>
                  <StatCard icon={Ic.chart} label="المنشورات"       value={fmt(insights.posts_created || 0)}  rawData={spark('posts_created')}     color="#00AAFF" trend={insights?.trends?.posts_created}/>
                  <StatCard icon={Ic.users} label="متابعون جدد"    value={fmt(insights.followers_gained || 0)} rawData={spark('followers_gained')}  color="#00AAFF" trend={3}/>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                  {[
                    { label: 'نسبة التفاعل',  pct: engRate, sub: `${engRate.toFixed(2)}% من الوصول`,      desc: 'التفاعل ÷ الوصول' },
                    { label: 'نسبة النقر',     pct: ctr,     sub: `${ctr.toFixed(2)}% من الإمبريشنز`,      desc: 'النقرات ÷ الإمبريشنز' },
                  ].map(m => (
                ...
                {aiText ? (
                  ...
                  <button onClick={explain} disabled={explaining}
                    ...
                    {explaining
                      ? <><div className="nexa-spinner" style={{ width: 12, height: 12 }}/>يقرأ أرقامك...</>
                      : <><span style={{ display: 'flex' }}>{Ic.bolt}</span>إيش تعني هذه الأرقام؟</>}
                  </button>
                )}
              </div>
            )}
            ...
            {tab === 'المحتوى' && (
              <div className="i-in">
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '16px' }}>
                  <StatCard icon={Ic.chart} label="المنشورات"            value={fmt(insights.posts_created || 0)}                                                                                rawData={spark('posts_created')} color="#00AAFF"/>
                  <StatCard icon={Ic.eye}   label="إجمالي الإمبريشنز"   value={fmt(insights.impressions)}                                                                                        rawData={spark('impressions')}   color="#00AAFF"/>
                  <StatCard icon={Ic.heart} label="إجمالي التفاعل"       value={fmt(insights.engagement)}                                                                                         rawData={spark('engagement')}    color="#00AAFF"/>
                  <StatCard icon={Ic.star}  label="متوسط لكل منشور"     value={insights.posts_created > 0 ? fmt(Math.round(insights.impressions / (insights.posts_created || 1))) : '—'} color="#22C55E"/>
                </div>
                <div style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', padding: '20px 24px', marginBottom: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                    <div>
                  ...
                  <div style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', padding: '20px 24px' }}>
                    ...
                    {raw.slice(-10).reverse().map((row, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '9px 0', borderBottom: i < 9 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                        ...
                        <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.70)', fontWeight: 600, minWidth: '55px', textAlign: 'left', fontFamily: MONO }}>{fmt(row.impressions || 0)}</div>
                        <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.30)', minWidth: '60px', textAlign: 'left', fontFamily: F }}>{fmt(row.engagement || 0)} تفاعل</div>
                        <div style={{ fontSize: '10px', color: PC[row.platform] || 'rgba(255,255,255,0.30)', fontWeight: 600, minWidth: '60px', textAlign: 'left', textTransform: 'capitalize', fontFamily: F }}>{row.platform || '—'}</div>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {tab === 'الإيميل' && (
              <div className="i-in">
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '16px' }}>
                  <StatCard icon={Ic.mail}  label="الإيميلات المرسلة" value={fmt(insights.email_sent   || 0)} rawData={spark('email_sent')}   color="#22C55E"/>
                  <StatCard icon={Ic.eye}   label="المفتوحة"            value={fmt(insights.email_opens  || 0)} rawData={spark('email_opens')}  color="#00AAFF"/>
                  <StatCard icon={Ic.click} label="النقرات"             value={fmt(insights.email_clicks || 0)} rawData={spark('email_clicks')} color="#00AAFF"/>
                  <StatCard icon={Ic.chart} label="نسبة الفتح"          value={pct(insights.email_opens || 0, insights.email_sent || 0)} color="#00AAFF"/>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                  {[
                    { label: 'نسبة الفتح',  pct: openRate,  sub: `${openRate.toFixed(1)}%`,  desc: 'المعدل العالمي: 21%',  good: openRate  > 21  },
                    { label: 'نسبة النقر',  pct: clickRate, sub: `${clickRate.toFixed(1)}%`, desc: 'المعدل العالمي: 2.3%', good: clickRate > 2.3 },
                  ].map(m => (
                    <div key={m.label} style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', padding: '20px', display: 'flex', alignItems: 'center', gap: '18px' }}>
                      ...
                      <div>
                        ...
                        <span style={{ fontSize: '10px', fontWeight: 700, padding: '3px 8px', borderRadius: '4px', fontFamily: F, background: m.good ? 'rgba(34,197,94,0.08)' : 'rgba(245,158,11,0.08)', border: `1px solid ${m.good ? 'rgba(34,197,94,0.18)' : 'rgba(245,158,11,0.18)'}`, color: m.good ? '#22C55E' : '#F59E0B' }}>
                          {m.good ? '↑ فوق المعدل' : '→ هناك مجال للتحسين'}
                        </span>
                      </div>
                    </div>
                  ))}
                ...
                {insights.email_sent === 0 && (
                  <div style={{ textAlign: 'center', padding: '48px', color: 'rgba(255,255,255,0.25)', fontSize: '13px', border: '1px dashed rgba(255,255,255,0.08)', borderRadius: '10px', fontFamily: F }}>
                    ما في بيانات إيميل بعد. ابدأ بإرسال تسلسلات عبر Automate.
                  </div>
                )}
              </div>
            )}
            ...
            {tab === 'المنصات' && (
              <div className="i-in">
                {connPlats.length > 0 ? (
                        ...
                        return (
                          <div key={p.platform} style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', padding: '20px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '6px', marginBottom: '12px' }}>
                              {[
                                { l: 'إمبريشنز', v: fmt(p.impressions) },
                                { l: 'تفاعل',    v: fmt(p.engagement)  },
                            ...
                            <div>
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.30)', fontFamily: F }}>نسبة التفاعل</span>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '50vh', textAlign: 'center', gap: '16px', fontFamily: F }}>
                    <div style={{ width: 52, height: 52, borderRadius: '14px', background: 'rgba(0,170,255,0.08)', border: '1px solid rgba(0,170,255,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(0,170,255,0.60)' }}>{Ic.chart}</div>
                    <div style={{ fontSize: '16px', fontWeight: 600, color: 'rgba(255,255,255,0.65)', fontFamily: F }}>ما في منصات متصلة بعد</div>
                    <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.35)', fontFamily: F }}>وصّل منصاتك الاجتماعية لتشاهد تحليلات كل منصة على حدة.</div>
