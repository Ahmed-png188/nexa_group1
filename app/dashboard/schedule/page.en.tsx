...
function Label({ children }: any) {
  return <div style={{ fontSize:'10px', fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase' as const, color:'rgba(0,170,255,0.70)', marginBottom:'10px', fontFamily:F_DEFAULT }}>{children}</div>
}
...
function Inner() {
  ...
  async function schedule() {
    ...
    try {
      ...
      if (r.ok) {
        setSaved(true); showToast(isArabic ? 'تم الجدولة — المنشور في القائمة' : 'Post scheduled — it\'s in the queue')
        setFBody('')
        setTimeout(()=>{ setModal(false); setSaved(false) },1400)
      } else showToast(isArabic ? 'صار خطأ — جرّب مرة ثانية' : 'Something went wrong — try again', false)
    } catch { showToast(isArabic ? 'صار خطأ' : 'Something went wrong', false) }
    setSaving(false)
  }
  ...
  async function publishNow(id:string) {
    try {
      ...
      if (r.ok) { showToast(isArabic ? 'تم النشر!' : 'Published!'); load() } else showToast('Failed', false)
    } catch {}
  }

  async function del(id:string) {
    await fetch(`/api/schedule-post?id=${id}`, { method:'DELETE' })
    showToast(isArabic ? 'تم الحذف' : 'Deleted'); load()
  }
    ...
    <>
      <div style={{ height:'calc(100vh - var(--topbar-h))', overflowY:'auto', background:'#0C0C0C', fontFamily:F }}>
        ...
        <div style={{
          ...
          <div>
            <h1 style={{ fontSize:'22px', fontWeight:700, letterSpacing:'-0.03em', color:'#FFFFFF', lineHeight:1, marginBottom:'6px', fontFamily:F }}>{isArabic ? 'الجدول' : 'Schedule'}</h1>
          ...
          <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
            ...
            <button onClick={() => { setModal(true); setFBody(''); setFDate(format(new Date(),'yyyy-MM-dd')) }}
              ...
              <span style={{ display:'flex' }}>{Ic.plus}</span>{isArabic ? 'منشور جديد' : 'New post'}
            </button>
          </div>
        </div>
                ...
                {upcoming.length > 0 ? (
                      ...
                      <div key={post.id} className="queue-row"
                        ...
                        <div style={{ display:'flex', gap:'5px', flexShrink:0, alignItems:'center' }}>
                          {post.platform === 'instagram' && (
                            <span style={{ fontSize:'10px', color:'rgba(255,255,255,0.28)', maxWidth:'120px', lineHeight:1.4, textAlign:'right', fontFamily:F }}>{isArabic ? 'انشر يدوياً على إنستغرام' : 'Post manually on Instagram'}</span>
                          )}
                          <button onClick={() => publishNow(post.id)}
                            ...
                            <span style={{ display:'flex' }}>{Ic.check}</span>{isArabic ? 'انشر الحين' : 'Publish now'}
                          </button>
                  ...
                  <div style={{ padding:'36px 24px', background:'#141414', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:'10px', textAlign:'center' }}>
                    <div style={{ fontSize:'14px', color:'rgba(255,255,255,0.30)', lineHeight:1.7, marginBottom:'20px', fontFamily:F }}>
                      {isArabic ? 'القائمة فاضية لحسابها.' : 'Nothing queued yet.'}<br/>
                    ...
                    <div style={{ display:'flex', gap:'8px', justifyContent:'center' }}>
                      <button onClick={() => { setModal(true); setFBody(''); setFDate(format(new Date(),'yyyy-MM-dd')) }}
                        ...
                        onMouseEnter={e => (e.currentTarget as HTMLElement).style.background='rgba(255,255,255,0.88)'}
                        onMouseLeave={e => (e.currentTarget as HTMLElement).style.background='#FFFFFF'}>
                        <span style={{ display:'flex' }}>{Ic.plus}</span>{isArabic ? 'جدوِل منشور' : 'Schedule a post'}
                      </button>
                    ...
                    <div key={plat} style={{ padding:'20px', background:'#141414', border:`1px solid ${conn?'rgba(255,255,255,0.14)':'rgba(255,255,255,0.07)'}`, borderRadius:'10px', transition:'all 0.15s' }}>
                      ...
                      {conn ? (
                        ...
                        <a href={`/api/oauth/${plat}?workspace_id=${ws?.id}`}
                          ...
                          <span style={{ display:'flex' }}>{Ic.plus}</span>{isArabic ? 'وصّل' : 'Connect'}
                        </a>
                      )}
                    </div>
                  )
        ...
        <div
          ...
          <div style={{ background:'#141414', border:'1px solid rgba(255,255,255,0.12)', borderTop:'2px solid #00AAFF', borderRadius:'14px', padding:'28px', width:'100%', maxWidth:'500px', boxShadow:'0 40px 100px rgba(0,0,0,0.90)', animation:'schUp 0.22s ease both', fontFamily:F }}>
            ...
            <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'24px' }}>
              <div>
                <div style={{ fontSize:'18px', fontWeight:600, letterSpacing:'-0.03em', color:'#FFFFFF', marginBottom:'4px', fontFamily:F }}>{isArabic ? 'جدوِل منشور' : 'Schedule a post'}</div>
                <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.30)', fontFamily:F }}>{isArabic ? '.احفظ في تقويمك — انشر يدوياً وقت ما تكون جاهز' : 'Save to your calendar — publish manually when ready.'}</div>
              </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', marginBottom:'20px' }}>
              {[
                { label: isArabic ? 'التاريخ' : 'Date', type:'date', value:fDate, set:setFDate },
                { label: isArabic ? 'الوقت' : 'Time', type:'time', value:fTime, set:setFTime },
              ].map(f => (
                <div key={f.label}>
                  <Label>{f.label}</Label>
            ...
            <div style={{ marginBottom:'24px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                <Label>{isArabic ? 'الكابشن' : 'Caption'}</Label>
              ...
              <div style={{ borderRadius:'10px', background:'rgba(255,255,255,0.03)', border:`1px solid ${fBody?'rgba(0,170,255,0.20)':'rgba(255,255,255,0.08)'}`, transition:'all 0.15s' }}>
                <textarea
                  ...
                  rows={6}
                  placeholder={`{isArabic ? 'اكتب أو الصق الكابشن هنا' : 'Write or paste your caption here'}.\n\nTip: Generate it first in Studio — it'll already be in your brand voice.`}
            ...
            <button onClick={schedule} disabled={!ready||saving}
              ...
              {saving
                ? <><div className="nexa-spinner" style={{ width:13, height:13 }}/>{isArabic ? 'جارٍ الجدولة...' : 'Scheduling…'}</>
                : saved
                ? <><span style={{ display:'flex' }}>{Ic.check}</span>{isArabic ? 'تم الجدولة!' : 'Scheduled!'}</>
                : <><span style={{ display:'flex' }}>{Ic.cal}</span>
                  {fDate ? (isArabic ? `جدوِل في ${format(new Date(fDate+'T12:00:00'),'MMM d')}` : `Schedule for ${format(new Date(fDate+'T12:00:00'),'MMM d')} at ${fTime}`) : isArabic ? 'أضف للقائمة' : 'Add to queue'}</>
              }
            </button>
          </div>
        </div>
