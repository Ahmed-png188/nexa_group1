...
function Field({ label, hint, children }: any) {
  return (
    <div style={{ marginBottom:20 }}>
      <div style={{ fontFamily:"'Geist', -apple-system, sans-serif", fontSize:12, fontWeight:500, color:'rgba(255,255,255,0.55)', marginBottom:7, letterSpacing:'-0.01em' }}>{label}</div>
      {children}
      {hint && <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)', marginTop:6, lineHeight:1.55 }}>{hint}</div>}
    </div>
  )
}
  ...
  return (
    <input
      ...
      style={{
        ...
        fontFamily:"'Geist', -apple-system, sans-serif", fontSize:14, outline:'none',
    ...
    <>
      <textarea
        ...
        style={{
          ...
          fontFamily:"'Geist', -apple-system, sans-serif", outline:'none', resize:'vertical',
        ...
        <div style={{ borderRight:'1px solid rgba(255,255,255,0.06)', display:'flex', flexDirection:'column', padding:'20px 12px', overflow:'hidden', background:'#141414' }}>
          ...
          <div className="card" style={{ padding:'14px 16px', marginBottom:16 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <div style={{ width:38, height:38, borderRadius:11, background:'linear-gradient(135deg, #00AAFF, rgba(0,170,255,0.5))', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Geist', -apple-system, sans-serif", fontSize:15, fontWeight:700, color:'#fff', flexShrink:0 }}>
                {initial}
              </div>
            ...
            {NAV.map(n => {
              ...
              return (
                <button key={n.id} onClick={() => { setTab(n.id); setSaved(false) }}
                  style={{ display:'flex', alignItems:'center', gap:9, padding:'9px 11px', borderRadius:10, background:on?'rgba(0,170,255,0.10)':'transparent', border:`1px solid ${on?'rgba(0,170,255,0.20)':'transparent'}`, color:on?'#00AAFF':'rgba(255,255,255,0.38)', cursor:'pointer', fontFamily:"'Geist', -apple-system, sans-serif", fontSize:13, fontWeight:on?600:400, transition:'all 0.15s', textAlign:'left', width:'100%' }}
          ...
          <button onClick={signOut}
            style={{ display:'flex', alignItems:'center', gap:8, padding:'9px 11px', borderRadius:10, background:'transparent', border:'1px solid transparent', color:'#EF4444', opacity:0.65, cursor:'pointer', fontSize:13, fontFamily:"'Geist', -apple-system, sans-serif", transition:'all 0.15s', width:'100%' }}
        ...
        <div style={{ overflowY:'auto', padding:'32px 36px 60px' }}>
          ...
          {tab === 'profile' && (
            <div style={{ maxWidth:520, animation:'pageUp 0.35s ease both' }}>
              <div style={{ marginBottom:28 }}>
                <h2 style={{ fontSize:20, fontWeight:700, letterSpacing:'-0.03em', color:'#FFFFFF', marginBottom:5 }}>Profile</h2>
                <p style={{ fontSize:12, color:'rgba(255,255,255,0.38)' }}>How you appear across your workspace</p>
              </div>
          ...
          {tab === 'workspace' && (
            <div style={{ maxWidth:560, animation:'pageUp 0.35s ease both' }}>
              <div style={{ marginBottom:28 }}>
                <h2 style={{ fontSize:20, fontWeight:700, letterSpacing:'-0.03em', color:'#FFFFFF', marginBottom:5 }}>Workspace</h2>
              ...
              <div style={{ marginBottom:20 }}>
                <div style={{ fontSize:11, fontWeight:600, color:'#00AAFF', letterSpacing:'0.06em', textTransform:'uppercase' as const, marginBottom:4 }}>Brand voice</div>
              ...
              <div style={{ marginBottom:20 }}>
                <div style={{ fontSize:11, fontWeight:600, color:'#00AAFF', letterSpacing:'0.06em', textTransform:'uppercase' as const, marginBottom:4 }}>Email sender identity</div>
          ...
          {tab === 'billing' && (
            <div style={{ animation:'pageUp 0.35s ease both' }}>
              <div style={{ marginBottom:28 }}>
                <h2 style={{ fontSize:20, fontWeight:700, letterSpacing:'-0.03em', color:'#FFFFFF', marginBottom:5 }}>Billing</h2>
                <p style={{ fontSize:12, color:'rgba(255,255,255,0.38)' }}>Manage your plan and credits</p>
              </div>

              {/* Current usage card */}
              <div className="card" style={{ marginBottom:24, display:'flex', alignItems:'center', gap:20 }}>
                <div>
                  <div style={{ fontSize:9, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase' as const, color:'rgba(255,255,255,0.38)', marginBottom:5 }}>Current plan</div>
                  <div style={{ fontSize:22, fontWeight:700, letterSpacing:'-0.04em', color:planColor, textTransform:'capitalize', fontFamily:"'Geist', -apple-system, sans-serif" }}>{currentPlan}</div>
                  {ws?.plan_status==='trialing' && (
                    <div style={{ marginTop:10, padding:'10px 14px', background:'rgba(245,158,11,0.10)', border:'1px solid rgba(245,158,11,0.20)', borderRadius:10 }}>
                      <div style={{ fontSize:11, fontWeight:600, color:'#F59E0B', marginBottom:3 }}>Trial active</div>
                  ...
                  {ws?.plan_status==='past_due' && (
                    <div style={{ marginTop:10, padding:'10px 14px', background:'rgba(239,68,68,0.10)', border:'1px solid rgba(239,68,68,0.20)', borderRadius:10 }}>
                      <div style={{ fontSize:11, fontWeight:600, color:'#EF4444', marginBottom:3 }}>Payment failed</div>
                      <div style={{ fontSize:11, color:'rgba(255,255,255,0.72)', lineHeight:1.55 }}>
                ...
                <div>
                  <div style={{ fontSize:10, fontWeight:600, letterSpacing:'0.06em', textTransform:'uppercase' as const, color:'rgba(255,255,255,0.38)', marginBottom:5 }}>Credits remaining</div>
                  <div style={{ fontSize:22, fontWeight:700, letterSpacing:'-0.04em', color:'#FFFFFF', fontFamily:"'Geist', -apple-system, sans-serif" }}>
                    {credits?.balance?.toLocaleString()||0}
                  </div>
                </div>
                <div style={{ width:1, height:44, background:'rgba(255,255,255,0.10)' }}/>
                <div>
                  <div style={{ fontSize:10, fontWeight:600, letterSpacing:'0.06em', textTransform:'uppercase' as const, color:'rgba(255,255,255,0.38)', marginBottom:5 }}>Lifetime used</div>
                  <div style={{ fontSize:22, fontWeight:700, letterSpacing:'-0.04em', color:'#FFFFFF', fontFamily:"'Geist', -apple-system, sans-serif" }}>
                      ...
                      {!isCurrent && (
                        <button onClick={() => checkout(plan.id)} disabled={billingLoading}
                          style={{ width:'100%', padding:'10px', fontSize:12, fontWeight:700,
                            ...
                            borderRadius:'10px', cursor:'pointer', fontFamily:"'Geist', -apple-system, sans-serif", transition:'all 0.15s', letterSpacing:'-0.01em',
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(130px,1fr))', gap:8 }}>
                  {([
                    ...
                    <button key={pack.credits}
                      ...
                      style={{ padding:'12px 10px', background:'#141414', border:`1px solid ${pack.tag?'rgba(0,170,255,0.25)':'rgba(255,255,255,0.10)'}`, borderRadius:10, cursor:'pointer', textAlign:'left' as const, position:'relative' as const, transition:'border-color 0.15s', opacity: billingLoading?0.6:1 }}
                      ...
                      <div style={{ fontSize:14, fontWeight:700, color:'#FFFFFF', letterSpacing:'-0.02em', marginBottom:2 }}>{pack.label}</div>
                      <div style={{ fontSize:11, color:'rgba(255,255,255,0.38)', marginBottom:6 }}>~{Math.floor(pack.credits/20)} vids or {Math.floor(pack.credits/3)} posts</div>
                      <div style={{ fontSize:13, fontWeight:700, color:'#00AAFF' }}>${pack.price}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ padding:'16px', background:'#141414', border:'1px solid rgba(255,255,255,0.10)', borderRadius:'10px' }}>
                <div style={{ fontSize:10, fontWeight:700, color:'rgba(255,255,255,0.38)', letterSpacing:'0.08em', textTransform:'uppercase' as const, marginBottom:12 }}>Credit costs per action</div>
          ...
          {tab === 'password' && (
            <div style={{ maxWidth:420, animation:'pageUp 0.35s ease both' }}>
              <div style={{ marginBottom:28 }}>
                <h2 style={{ fontSize:20, fontWeight:700, letterSpacing:'-0.03em', color:'#FFFFFF', marginBottom:5 }}>Security</h2>
                <p style={{ fontSize:12, color:'rgba(255,255,255,0.38)' }}>Update your password</p>
              </div>
              ...
              <div style={{ marginTop:40, paddingTop:28, borderTop:'1px solid rgba(239,68,68,0.20)' }}>
                <div style={{ fontSize:13, fontWeight:600, color:'#EF4444', opacity:0.8, marginBottom:5 }}>Danger zone</div>
