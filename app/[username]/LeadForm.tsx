...
const FONTS:Record<string,string> = {
  geist:"'Geist',-apple-system,sans-serif", inter:"'Inter',system-ui,sans-serif",
  dm:"'DM Sans',sans-serif", mono:"'Geist Mono',monospace", serif:"Georgia,'Times New Roman',serif",
}
...
export default function LeadForm({workspace}:{workspace:Workspace}) {
  ...
  try { if (workspace.lead_page_fields) fields=JSON.parse(workspace.lead_page_fields) } catch{}

  const [values,   setValues]   = useState<Record<string,string|string[]>>(Object.fromEntries(fields.map(f=>[f.id, f.type==='multiselect'?[]:'' ])))
  ...
  const t  = THEMES[workspace.lead_page_theme||'dark']||THEMES.dark
  const ff = FONTS[workspace.lead_page_font||'geist']||FONTS.geist
  const a  = workspace.lead_page_accent||'#00AAFF'
  const isLight = ['light','warm'].includes(workspace.lead_page_theme||'dark')

  const headline = workspace.lead_page_headline||(brandName?`Get in touch with ${brandName}`:'Get in touch')
  const subline  = workspace.lead_page_subline ||"Drop your details and we'll be in touch soon."
  const cta      = workspace.lead_page_cta||(SEGMENT_CTA[workspace.segment||'business']||'Send →')
  ...
  const inp=(extra?:React.CSSProperties):React.CSSProperties=>({
    ...
    border:`1px solid ${t.border}`,borderRadius:10,color:t.text1,
    fontFamily:ff,outline:'none',transition:'border-color 0.15s',boxSizing:'border-box',...extra,
  })
  ...
  async function handleSubmit() {
    ...
    if (!emailVal||!emailVal.includes('@')) { setError('Please enter a valid email address'); return }

    setSubmitting(true); setError('')
    try {
      const res = await fetch('/api/lead-capture',{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({workspace_id:workspace.id,form_values:values,fields,source:'lead_page'}),
      })
      if (res.ok) {
  ...
  function renderField(field:FormField) {
    ...
    if (field.type==='textarea') return (
      <textarea value={val as string} onChange={e=>setValue(field.id,e.target.value)} placeholder={field.placeholder||''}
        rows={3} style={{...inp(),resize:'vertical' as const,lineHeight:1.65}}/>
    )
    if (field.type==='select') return (
      <select value={val as string} onChange={e=>setValue(field.id,e.target.value)}
      ...
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        {(field.options||[]).map(o=>(
          <label key={o} style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer'}}>
            <div onClick={()=>setValue(field.id,o)} style={{width:18,height:18,borderRadius:'50%',border:`2px solid ${val===o?a:t.border}`,background:val===o?a:'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,transition:'all 0.15s',cursor:'pointer'}}>
              {val===o&&<div style={{width:7,height:7,borderRadius:'50%',background:'#000'}}/>}
            </div>
            <span style={{fontSize:14,color:t.text1,fontFamily:ff}}>{o}</span>
          </label>
        ))}
      </div>
    )
    if (field.type==='multiselect') return (
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        {(field.options||[]).map(o=>{
          ...
          return (
            <label key={o} style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer'}}>
              <div onClick={()=>toggleMulti(field.id,o)} style={{width:18,height:18,borderRadius:4,border:`2px solid ${checked?a:t.border}`,background:checked?a:'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,transition:'all 0.15s',cursor:'pointer'}}>
    ...
    if (field.type==='checkbox') return (
      <label style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer'}}>
        <div onClick={()=>setValue(field.id,val?'':'checked')} style={{width:18,height:18,borderRadius:4,border:`2px solid ${val?a:t.border}`,background:val?a:'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,transition:'all 0.15s',cursor:'pointer'}}>
        ...
        <span style={{fontSize:14,color:t.text1,fontFamily:ff}}>{field.label}</span>
      </label>
    )
    return (
  ...
  return (
    <div style={{minHeight:'100vh',background:t.bg,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 20px',position:'relative',overflow:'hidden',fontFamily:ff}}>
      ...
      <div className="lf-in" style={{position:'relative',zIndex:1,width:'100%',maxWidth:420,textAlign:layout==='left'?'left':'center'}}>

        {/* Logo / avatar */}
        <div style={{marginBottom:24, paddingLeft: layout==='left'?'24px':0, paddingRight: layout==='left'?'24px':0}}>
          {workspace.lead_page_logo_url
            ?<img src={workspace.lead_page_logo_url} alt={brandName} style={{width:64,height:64,borderRadius:16,objectFit:'cover',display:layout==='left'?'block':'inline-block',marginBottom:16,border:`1px solid ${t.border}`}}/>
            :<div style={{width:64,height:64,borderRadius:18,background:`${a}20`,border:`1px solid ${a}35`,display:'flex',alignItems:'center',justifyContent:'center',margin:layout==='left'?'0 0 16px':'0 auto 16px',fontSize:24,fontWeight:700,color:a}}>
              {(brandName||'N')[0].toUpperCase()}
            </div>
          }
          <h1 style={{fontSize:24,fontWeight:700,color:t.text1,letterSpacing:'-0.03em',marginBottom:8,lineHeight:1.2,fontFamily:ff}}>{headline}</h1>
          <p style={{fontSize:14,color:t.text2,lineHeight:1.7}}>{subline}</p>
        </div>

        {/* Form */}
        <div style={{background:t.surface,border:`1px solid ${t.border}`,borderRadius:18,padding:'28px 24px',position:'relative',overflow:'hidden',boxShadow:`0 24px 64px rgba(0,0,0,0.3),0 0 0 1px ${a}08`,textAlign:'left'}}>
          ...
          {submitted?(
            <div style={{textAlign:'center',padding:'24px 0'}}>
              ...
              <div style={{fontSize:22,fontWeight:700,color:t.text1,marginBottom:8,letterSpacing:'-0.03em'}}>You&apos;re in.</div>
              <div style={{fontSize:14,color:t.text2,lineHeight:1.65,marginBottom:magnetUrl?20:0}}>Expect to hear from {brandName} soon.</div>
              {magnetUrl&&(
                <a href={magnetUrl} target="_blank" rel="noreferrer"
                  style={{display:'inline-flex',alignItems:'center',gap:9,padding:'12px 24px',background:a,color:isLight?'#fff':'#000',borderRadius:10,textDecoration:'none',fontWeight:700,fontSize:14,marginTop:4}}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                  {workspace.lead_magnet_label||'Download your resource'}
                </a>
              )}
            </div>
          ):(
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              {fields.map(field=>(
                <div key={field.id}>
                  {field.type!=='checkbox'&&(
                    <div style={{fontSize:11,fontWeight:600,letterSpacing:'0.06em',textTransform:'uppercase' as const,color:t.text3,marginBottom:7,lineHeight:1.3,textAlign:'left' as const}}>
                      {field.label}{field.required&&<span style={{color:a,marginLeft:3}}>*</span>}
                    </div>
                  )}
                  {renderField(field)}
                </div>
              ...
              {workspace.lead_magnet_enabled&&workspace.lead_magnet_url&&(
                <div style={{padding:'10px 14px',background:`${a}10`,border:`1px solid ${a}25`,borderRadius:9,display:'flex',alignItems:'center',gap:9}}>
                  ...
                  <span style={{fontSize:12,fontWeight:600,color:a}}>{workspace.lead_magnet_label||'Free resource unlocked after submission'}</span>
                </div>
              )}

              {error&&<div style={{fontSize:12,color:'#EF4444',padding:'9px 12px',background:'rgba(239,68,68,0.08)',border:'1px solid rgba(239,68,68,0.18)',borderRadius:8}}>{error}</div>}

              <button onClick={handleSubmit} disabled={submitting}
                style={{width:'100%',padding:'13px',background:btnBg,color:btnColor,border:btnBorder,borderRadius:10,fontFamily:ff,fontSize:14,fontWeight:700,cursor:'pointer',transition:'all 0.18s',letterSpacing:'-0.01em',boxShadow:workspace.lead_page_button_style==='filled'?`0 4px 18px ${a}35`:'none'}}>
        ...
        {showPwr&&(
          <div style={{textAlign:'center',marginTop:18,fontSize:11,color:t.text3}}>
            Powered by <a href="https://nexaa.cc" style={{color:a,textDecoration:'none',fontWeight:600,opacity:0.8}}>nexaa.cc</a>
          </div>
        )}
      </div>
    </div>
